...
영여 원문의 `motion` 은 현재 우리가 운동량 $\boldsymbol{p}= m\boldsymbol{v}$ 으로 부르는 값이다. 즉 외부에서 힘 $\boldsymbol{F}$ 가 가해질 때 뉴튼의 제2 운동법칙은 다음을 말한다. 
...
이 $m$ 을 **관성질량(inerital mass)** 이라고 한다. 

또 하나 언급해야 할 것은 $\boldsymbol{F}=\boldsymbol{0}$ 일 경우 운동량 $\boldsymbol{p}$ 는 일정하게 유지된다. 이것을 **운동량 보존** 이라고 한다.
...
여기서의 *action* 은 한 물체에서 다른 물체에 대해 힘을 가하는 것을 말한다. 즉 두 물체 사이 $A$ 와 $B$ 사이에 힘이 작용할 때 $A$ 가 $B$ 의 존재로 인해 받는 힘 $\boldsymbol{F}_{BA}$ 와 $B$ 가 $A$ 의 존재로 인해 받는 힘 $\boldsymbol{F}_{AB}$ 사이에는 $\boldsymbol{F}_{BA}=-\boldsymbol{F}_{AB}$ 의 관계가 성립한다는 의미이다. 
...
$$ 
\dot{\boldsymbol{L}} = \sum_i \dfrac{d}{dt}( \boldsymbol{r}_i \times \boldsymbol{p}_i) = \sum_i \dot{\boldsymbol{r}}_i \times \boldsymbol{p}_i + \boldsymbol{r}_i \times \dot{\boldsymbol{p}}_i = \sum_i \boldsymbol{r}_i \times \boldsymbol{F}^{(e)}_i +  \sum_{\substack{ i ,j \\ i \ne j }} \boldsymbol{r}_i \times \boldsymbol{F}_{ji} 
...
$$
\begin{aligned}
\boldsymbol{L} = \sum_i \boldsymbol{r}_i \times m_i \boldsymbol{v}_i  &=\sum_i \boldsymbol{R} \times m_i \boldsymbol{V} + \sum_i \boldsymbol{r}'_i  \times m_i \boldsymbol{v}'_i + \left(\sum_i m_i \boldsymbol{r}'_i \right) \times \boldsymbol{V} + \boldsymbol{R} \times \left( \dfrac{d}{dt} \sum_i m_i \boldsymbol{r}'_i\right)   \\
...
::: {.callout-caution appearance="minimal" icon="false"}
원점 $\mathcal{O}$에서의 입자계의 총 각운동량 $\boldsymbol{L}$은 1) 모든 질량이 center of mass 에 집중되어 있다고 가정할 때의 각운동량 $\boldsymbol{L}_C$ 와 2) center of mass를 origin으로 했을때의 각운동량 $\boldsymbol{L}_R$ 의 합이다. 
:::
...
이므로 $\boldsymbol{F}_{ji}$ 와 $\boldsymbol{F}_{ij}$ 모두 두 입자의 위치를 잇는 선상에 위치한다. 즉 어떤 scalar function $f$ 에 대해, $V_{ij}$ 는 다음과 같은 form이다.
...
이제 constraint force 에 의한  *virtual work* 이 $0$ 인 시스템, 즉 $\sum_i \boldsymbol{f}_i \cdot \delta \boldsymbol{r}_i=0$ 인 시스템으로 제한해보자. 이 조건은 강체를 비롯한 많은 다른 제약 조건에 대해서도 유효하다는 것을 안다. 입자가 표면 위를 움직이도록 제약되어 있다면, constraint force 는 표면에 수직이며, virtural displacement 는 표면에 접선이어야 하므로 virtual work 은 $0$ 이다. 하지만 슬라이딩 마찰력이 존재할 경우는 virtural work 이 $0$ 이 아니며, 이런 시스템은 지금은 배재한다. 이 제한은 마찰이 본질적으로 macroscopic 한 현상이라는 것을 고려하면 우리의 물리를 해치지 않는다. 반면에 구르는(rolling) constraint 는 이 조건을 위반하지 않는다. 왜냐하면 잠시 멈춘 지점에 작용하는 힘은 constraint 에 일관된 변위에 대해 virtual work 이 $0$ 이기 때문이다. 또한, 입자가 시간에 따라 움직이는 표면에 제약되어 있는 경우, constraint force 는 각 순간마다 표면에 수직이며, virtual work 은 $0$ 이지만 실제 시간 $dt$ 동안의 real displacement 에 의한 일은 $0$ 이 아닐 수 있다.
...
를 통해 **일반화된 힘(generalized force)** $Q_j = \sum\limits_i \boldsymbol{F}_i \cdot \dfrac{\partial \boldsymbol{r}_i}{\partial q_j}$ 를 정의한다. 일반화된 좌표계에서 $q_j$ 가 거리의 차원을 갖지 않아도 되기 때문에 $Q_j$ 역시 힘과 같은 차원을 가질 필요는 없다. 다만 $Q_j \delta q_j$ 는 일(Work)과 같은 차원을 갖는다.
...
Holonomic constraints 조건에서는 독립적인 일반화된 자표 $q_1,\ldots,\,q_n$ 을 찾을 수 있다. 그렇다면 @eq-dalembert_principle2 는 각각의 $q_j$ 에 대해,
...
::: {.callout-caution appearance="minimal" icon="false"}
The equation of motion in the form (@eq-lagrange_2) are not necessarily restricted to conservative systems; only if $V$ is not an explicit function of time is the system conservative.
:::


지금까지는 $V$ 가 generalized velocity에 의존하지 않는다고 가정하였다. 이 경우 *Lagrangian* $L$ 을 $L=T-V$ 로 정의한다면,

$$\
\dfrac{d}{dt} \left( \dfrac{\partial (T-V)}{\partial \dot{q}_j} \right) - \dfrac{\partial (T-V)}{\partial q_j} = \dfrac{d}{dt} \left( \dfrac{\partial L}{\partial \dot{q}_j} \right) - \dfrac{\partial L}{\partial q_j} = 0
$$ {#eq-Lagrange}
...
Lagrangian $L(q,\,\dot{q},\,t)$ 와 어떤 미분가능한 함수 $F(q,\,t)$ 에 대해, $L'(q,\,\dot{q},\,t) = L(q,\,\dot{q},\,t) + \dfrac{dF}{dt}$ 는 $L(q,\,\dot{q},\,t)$ 와 같은 equation of motion, 즉 Lagrange's equation 을 낳는다. 즉 어떤 System에 대한 Lagrangian은 unique 하지 않다. 
...
만약 generalized forces 가 어떤 함수 $U(q_j,\,\dot{q}_j)$ 에 대해
...
### generalize potential for electromagnetic forces

Generalized potential $U$ 가 다음과 같이 주어졌다고 하자.

$$
U=q \phi - \dfrac{q}{c} \boldsymbol{A} \cdot \boldsymbol{v}\;.
$$ {#eq-electromagnetic_potential}

이. $U$ 에 대한 generalize force가 Lorentz force 임을 보일 수 있다.
...
&= \sum_i\left[\left(\dfrac{\partial^2 T}{\partial q_i \partial \dot{q}_j}\right) \dot{q}_i  + \dfrac{\partial T}{\partial q_i} \delta_{ij}+  \left(\dfrac{\partial^2 T}{\partial \dot{q}_i \partial \dot{q}_j}\right) \ddot{q}_i  + \left(\dfrac{\partial T}{\partial \dot{q}_i}\right)\left(\dfrac{\partial \ddot{q}_i}{\partial \dot{q}_j}\right)  + \left(\dfrac{\partial^2 T}{\partial t \partial \dot{q}_j}\right)\right] \\
&= \dfrac{d}{dt}\left(\dfrac{\partial T}{\partial \dot{q}_j}\right) + \dfrac{\partial T}{\partial q_j} +  \sum_i \left(\dfrac{\partial T}{\partial \dot{q}_i}\right)\left(\dfrac{\partial \ddot{q}_i}{\partial \dot{q}_j}\right) 
\end{aligned}
$$
...
$$
\begin{aligned}
\dfrac{\partial \ddot{q}_i}{\partial \dot{q}_j} &= \dfrac{\partial}{\partial \dot{q}_j} \left(\dfrac{d \dot{q}_i}{dt}\right)= \dfrac{\partial}{\partial \dot{q}_j} \left[\sum_k  \dfrac{\partial \dot{q}_i}{\partial q_k} \dot{q}_k + \sum_k \dfrac{\partial \dot{q}_i}{\partial \dot{q}_k}\ddot{q}_k + \dfrac{\partial \dot{q}_i}{\partial t} \right] = \delta_{ij} + 
\end{aligned}
$$
:::