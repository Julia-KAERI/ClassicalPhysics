...
고전역학에서 **입자(particle)** 는 질량을 가지고 있지만 부피가 없는, 혹은 우리가 측정하고자 하는 양에 대해 무시할만한 어떤 가상의 물리적 존재이다. 공간상에 존재하며 시간의 변화에 따라 공간상에서 움직이며, 시간에 따른 위치를 **경로(path 혹은 trajectory)** 라고 한다. 좌표계에서 원점 $\mathcal{O}$ 에 대한 변위를 **radius vector** 라고 한다. 고전역학에서 다루는 공간은 3차원 공간이며, 원점 $\mathcal{O}$ 에 대해 임의의 표준 기저 $\hat{\boldsymbol{e}}_1,\,\hat{\boldsymbol{e}}_2,\,\hat{\boldsymbol{e}}_3$ 를 관례적으로 $\hat{\boldsymbol{e}}_3 = \hat{\boldsymbol{e}}_1 \times \hat{\boldsymbol{e}}_2$ 가 되도록 잡아 radius vector $\boldsymbol{r}$ 을 이 표준기저에 대한 좌표로 표현한다. 즉 $\boldsymbol{r} = x\hat{\boldsymbol{e}}_1 + y \hat{\boldsymbol{e}}_2 + z\hat{\boldsymbol{e}}_3$ 일 때 $\boldsymbol{r}$ 을 $(x,\,y,\,z)$ 혹은 $\begin{bmatrix} x & y & z\end{bmatrix}^T$ 로 표현하며 이를 **좌표(coordinate)** 라고 한다.

![두 좌표계](../images/classical mechanics/fig-refrence_frame-1.png){#fig-reference_frames width="70%"}


원점과 표준기저가 정해졌을 모든 위치의 좌표가 정해진다. 이렇게 위치를 특정하기 위해 정해진 원점과 표준기저를 **좌표계(reference frame)** 이라고 한다. 그런데 원점과 표준기저는 임의적이다. 즉 원점 자체는 공간상의 어느 위치에다 정해도 되며, 정해진 원점에 대해서도 가능한 표준기저는 무수히 많다. 즉 좌표계는 개수는 무한대이다. 또한 시간이라는 다른 변수도 존재한다. 하나의 원점 ($\mathcal{O}_1$)에 대해 다른 좌표계의 원점($\mathcal{O}_2$)은 시간에 따라 그 위치가 바뀔 수도 있다. $\mathcal{O}_2$ 의 좌표가 $\mathcal{O}_1$ 으로 기술되는 좌표계에서 시간에 따라 일정하게 변할수도 있고, 시간의 제곱에 따라 변할수도 있다. 

</br>

어떤 좌표계를 사용하더라도 유지되어야 할 기본적인 량이 있다. 하나는 거리이다. 즉 공간상의 두 점 $P_1,\, P_2$ 사이의 거리는 어떤 좌표계에서도 동일해야 한다. 다른 하나는 시간이다. 두 사건의 시간상의 차이 $|t_2-t_1$ 은 어느 좌표계에서든 같야야 한다. 이것은 뒤에 나올 상대성 이론과의 가장 큰 차이점이자, 실제로 고전역학에서는 너무도 당연하여 언급하지 않고 넘어가기도 한다. 
...
즉 관성 좌표계 사이에서는 동일한 운동방정식이 존재한다. 이를 **갈릴레이의 상대성 원리 (Galilean relativity)** 라고 한다.
