...
어떤 좌표계를 사용하더라도 유지되어야 할 기본적인 량이 있다. 하나는 거리이다. 즉 공간상의 두 점 $P_1,\, P_2$ 사이의 거리는 어떤 좌표계에서도 동일해야 한다. 다른 하나는 시간이다. 두 사건의 시간상의 차이 $t_2-t_1$ 은 어느 좌표계에서든 같야야 한다. 이것은 뒤에 나올 상대성 이론과의 가장 큰 차이점이다.
...
우리는 앞서 관성좌표계를 정의했다. 만약 입자가 정지를 포함한 등속운동 상태이지 않으면 무언가가 입자에 영향을 준다는 의미이며 그것을 **힘(force)** 라고 하고 정량적으로는 운동량 $\boldsymbol{p}:= m\boldsymbol{v}$ 의 시간에 대한 변화량으로 정의한다. 영어 표현의 `motion` 은 현재 우리가 운동량  으로 부르는 값이다. 즉 외부에서 힘 $\boldsymbol{F}$ 가 가해질 때 뉴튼의 제2 운동법칙은 다음을 말한다. 
...
이 $m$ 을 **관성질량(inerital mass)** ($^\ast$) 이라고 한다.[$^\ast$ 관성질량과 비교되는 값은 중력질량이다. 이것은 만유인력을 다룰 때 설명한다.]{.aside} $\boldsymbol{F}=\boldsymbol{0}$ 일 경우 운동량 $\boldsymbol{p}$ 는 일정하게 유지된다. 이것을 **운동량 보존** 이라고 한다.
