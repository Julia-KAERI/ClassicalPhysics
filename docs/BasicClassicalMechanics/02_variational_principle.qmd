...
이 stationary value를 갖도록 하는 경로를 따른다. 다시 말하자면  $t_1$ 에서의 위치 $\boldsymbol{x}_1$ 에서 $t_2$ 에서의 위치 $\boldsymbol{x}_2$ 사이의 모든 가능한 경로중 실제 자연법칙에 의해 선택되는 경로는 @eq-action 이 stationary value를 갖도록 하는 경로이다.
:::


선적분에 stationary value 를 갖는다는 것은 
