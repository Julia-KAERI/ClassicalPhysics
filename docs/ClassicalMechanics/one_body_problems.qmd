...
## 1차원 Drag force {#sec-1d_drag_force}
...
이다. $|t| \ll 1$ 이면 $v(t) = gt$ 이다. 즉 이 물체는 정지상태에서 시작하기 때문에 $|t| \\ 1$ 인 경우에는 자유낙하와 비슷하게 움직이게 된다. $t\gg tau$ 인 경우 $e^{-t/\tau} \to 0$ 이므로 $\tau g$ 의 등속운동을 하게 된다.
...
($c$) spherical shell 이 아니라 sphere 이며 sphecally symmetric 할 때 구 내부의 원점으로부터의 거리 $R$ 인 지점의 중력장은 구의 중심을 향하며 그 크기가 $GM(R)/R^2$ 임을 보여라. 여기서 $M(R)$ 은 반경 $R$ 보다 구에서 반지름 $R$ 인 부분의 질량이다. 즉 $R$ 바깥의 구체의 질량은 중력장에 영향을 끼치지 않는다.
...
($c$) 구(sphere) 를 spherical shell 의 쌓임으로 이해하자. 구 내부의 반경 $R$ 인 지점에서 반경 $R$ 보다 큰 위치에서의 구의 부분에 대한 중력의 기여는 상수이며 따라서 $R$ 에서의 중력에 기여하지 않는다. 이제 반지름이 $0\to R$ 까지 변한다고 할 때, $\Delta M = 4\pi \rho a^2 \Delta a$ 이므로

$$
V_\textrm{sphere}(R) = - Gm\rho \int_0^R \dfrac{4\pi r^2 dr}R dr  = -\dfrac{4\pi Gm\rho R^2}{3} = - \dfrac{GmM(R)}{R}
$$
...
균일한 질량 밀도 $\rho(t)$ 를 갖는 거대한 구체의 먼지 분포를 생각하자. 즉 질량 밀도는 공간적으로는 균일하지만 시간에 대해서는 변화한다. 이 구체가 팽창하면 시간에 대해 감소할 것이고 이 구채가 수축하면 시간에 대해 증가한다. $t=0$ 일 때 이 구체는 한 점(여기서 원점으로 삼는다)에 집중되었으며, 밀도는 무한이고, 외부로 향하는 속도 역시 무한이다. 즉 빅뱅이 일어났다. 어떤 시간 $t_0$ 에서 밀도는 $\rho_0$ 이며 원점에서의 거리가 $r_0$ 인 한 특정 먼지의 외부 방향 속력이 $v_0$ 라고 하자. 
