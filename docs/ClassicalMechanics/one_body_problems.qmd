...
1차원 단순조화진동자(simple harmonic osciilator, 이하 SHO)의 경우 운동방정식이 $m\ddot{x} = -kx$ 이 며 그 해는 $\omega_0 :=\sqrt{\dfrac{k}{m}}$ 에 대해 $x(t) = A\cos (\omega_0 t+\phi_0)$ 라는 것을 안다. 이 때 $A$ 와 $\phi_0$ 는 초기조건 $x(t_0),\, v(t_0)$ 에 의해 정해지는 상수이다. 
