...
이다. 그렇다면 일반해는 @eq-characteristic_solution_of_dho_for_overdampled, @eq-characteristic_solution_of_dho_for_underdampled, @eq-characteristic_solution_of_dho_for_criticallly_damped 가운데 해당되는 경우와 @eq-particular_solution_of_ddho 의 합으로 표현된다. 앞서 언급했듯이 $t\gg 1$ 이면 일시적인 해는 $0$ 으로 수렴하기 때문에 $x_p(t)$ 가 주도적이 된다. 이를 반영하여 $x_p(t)$ 를 **안정한 해 (steady solution)** 이라고 한다. 흥미로운 것은 외부 힘의 진동수 $\omega$ 와 조화진동자의 고유진동수 $\omega_0$ 가 가까울수록 $x_p(t)$ 의 진폭이 커지며 $\omega = \omega_0$ 라면 진폭이 발산하게된다. 이를 **공명(resonance)** 라고 한다. $x_0 (\omega) =\dfrac{f_0}{\sqrt{(\omega_0^2-\omega^2)^2+4\beta^2\omega^2}}$ 라고 할 때 $\omega/\omega_0$ 에 대한 $x_0(\omega)$ 는 @fig-resonance 와 같다. 

![Resonance](../images/classical%20mechanics/fig-resonance.png){#fig-resonance width=340}
...
$$
\dfrac{1}{2}{\dot{a}}^2 - \dfrac{GM/{R_0}^2}{a} = \text{const.} = \epsilon.
$$
