...
$m_1<m_2$ 일 때는 $AO < mv$ 이며 $m_1>m_2$ 일 때는 $AO > mv$ 이다. $\theta_1,\,\theta_2$ 는 충돌 이후 $m_1$ 과 $m_2$ 의 이동 각도이다. 이제 질량중심좌표계에서의 산란각 $\chi$ 와 $\theta_1,\,\theta_2$ 의 관계는 @fig-elastic_scattering_angles_ss 로 부터 다음과 같음을 쉽게 계산 할 수 있다. 

$$
\begin{aligned}
\tan \theta_1 &= \dfrac{|OC| \sin \chi}{|AO| + |OC|\cos \chi} = \dfrac{mv \sin \chi}{\dfrac{m_1^2}{m_1+m_2}v + mv\sin \chi} = \dfrac{m_2 \sin \chi}{m_1 + m_2 \sin \chi}, \\
\theta_2 &= \dfrac{1}{2}(\pi - \chi).
\end{aligned}
...
$$
\begin{aligned}
v_1' &= \dfrac{p_1'}{m_1} = \dfrac{\sqrt{|AO|^2 + |OC|^2 - 2|AO||OC| \cos (\pi - \chi)}}{m_1} \\
&= \dfrac{\sqrt{{m_1}^2 + {m_2}^2 + 2m_1m_2 \cos \chi}}{m_1+m_2}v, \\
v_2' &= \dfrac{p_2'}{m_2} = \dfrac{\sqrt{|OC|^2+|OB|^2-2|OC||OB|\cos \chi}}{m_2} \\
&= \dfrac{2m_1v}{m_1+m_2} \sin \left(\frac{1}{2}\chi\right).
\end{aligned}
...
두 입자가 충돌 후 같은 방향 혹은 반대 방향을 향하는 것을 **정면 충돌(head-on collison)** 이라고 한다. 즉 $\chi = \pi$ 가 된다. 정면충돌의 경우 충돌 후의 속도는 @eq-elastic_scattering_velocity_in_lab_frame 으로부터 알 수 있으며,
...
$$
\phi_0 = \int_{r_{\min}}^\infty \dfrac{l/r^2}{\sqrt{2m(E-V(r)) - l^2/r^2}}\, dr
$$

이며, $r_\min$ 은 적분 분모의 $\sqrt{\cdots}$ 를 $0$ 으로 한다는 것을 기억하자. 
