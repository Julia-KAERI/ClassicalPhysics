...
$$ {#eq-part2_laplace_equation_in_spherical_coordinate}
...
$${#eq-part2_laplace_equation_in_spherical_coordinate_2}
...
$$ {#eq-part2_generator_of_legendre_polymonals}

두 벡터 $\bf{r}_>\, \bf{r}_<$ 이 $\|\bf{r}_>\| > \|\bf{r}_<\|$ 라고 하자. $t=\dfrac{\|\bf{r}_<\|}{\|\bf{r}_>\|}$ 에 대해 

$$
\begin{aligned}
\dfrac{1}{\|\bf{r}_>-\bf{r}_<\|} &=  \dfrac{1}{\|\bf{r}_>\|} \dfrac{1}{\sqrt{1-2\cos \theta t+t^2}} = \dfrac{1}{\|\bf{r}_>\|}\sum_{n=0}^\infty \left(\dfrac{\|\bf{r}_<\|}{\|\bf{r}_>\|}\right)^n P_n (\cos \theta)
\end{aligned}
...
$$ {#eq-part2_orthogonality_of_legendre_polynomial}

$$
P_n(-x) = (-1)^n P_n(x)
$${#eq-part2_parity_of_legendre_polynomial}
...
\end{aligned}
$$ {#eq-part2_definition_of_associated_legendre_polynomial}
...
$$ {#eq-part2_orthogonality_of_associated_legendre_polynomials}
...
$$ {#eq-part2_definition_of_spherical_harmomics}
...
$$
\oint_{\Omega} {Y_{l'}^{m'}}^\ast (\theta,\, \varphi)\, Y_l^m (\theta,\, \varphi) d\Omega =\int_0^{2\pi} \int_0^\pi d\theta  {Y_{l'}^{m'}}^\ast (\theta,\, \varphi)\, Y_l^m (\theta,\, \varphi)  \sin \theta\, d\theta d\varphi= \delta_{ll'}\delta_{mm'}
$$ {#eq-part2_orthogonamity_of_spherical_harmonics}
...
$$ {#eq-part2_similarity_of_spherical_harmonics}
...
$$ {#eq-part2_completeness_of_spherical_harmonics}
...
$$ {#eq-part2_addition_theorem_for_spherical_harmonics}

임을 보일 수 있다. 그렇다면 @eq-part2_legendre_expansion 로부터

$$
\dfrac{1}{\|\bf{r}_> - \bf{r}_<\|} = \sum_{l=0}^\infty \sum_{m=-l}^l \dfrac{4\pi}{2l+1} \left(\dfrac{\|\bf{r}_<\|^l}{\|\bf{r}_>\|^{l+1}}\right) {Y_l^m}^\ast(\theta,\,\varphi') Y_l^m(\theta,\, \varphi)
$$ {#eq-part2_spherical_haromics_expansion_of_inverse_vector_difference}
...
전하 $+q$ 와 $-q$ 가 위의 그림과 같이 $d$ 의 거리만큼 떨어져 있다고 하고 원점을 두 전하의 중앙에 두자. 그리고 $+q$ 로부터의 변위를 $\bf{r}_1$, $-q$ 로부터의 변위를 $\bf{r}_2$ 라고 하고 원점으로부터의 변위를 $\bf{r}$ 라고 하자. 그리고 $-q$ 전하의 위치로부터 $+q$ 전하의 위치까지의 변위를 $\bf{d}$ 라고 하자. 이 때의 전기 포텐셜 $\phi (\bf{r})$ 은 $r=\|\bf{r}\|$ 이라고 할 때 $r>d$ 의 조건과 @eq-part2_legendre_expansion 와 @eq-part2_parity_of_legendre_polynomial 로부터 다음을 얻는다.
$$
\begin{aligned}
\phi(\bf{r}) &= \dfrac{1}{4\pi\epsilon_0}\left[\dfrac{q}{\|\bf{r}_1\|} - \dfrac{q}{\|\bf{r}_2\|}\right] = \dfrac{1}{4\pi\epsilon_0} \left[\dfrac{1}{\|\bf{r}-\bf{d}/2\|} - \dfrac{1}{\|\bf{r}+\bf{d}/2\|}\right] \\[0.3em]
&= \dfrac{1}{4\pi\epsilon_0} \sum_{k=0}^\infty \dfrac{d^k}{2^kr^{k+1}} \left[P_k(\cos\theta)- P_k (-\cos\theta)\right] \\
&=\dfrac{1}{4\pi\epsilon_0} \sum_{k=0}^\infty \dfrac{2d^{2k+1}}{2^{2k+1}r^{2k+2}} P_{2k+1}(\cos\theta) \\
&= \dfrac{1}{4\pi\epsilon_0} \left[\dfrac{d\cos\theta}{r^2} + \dfrac{d^3(5\cos^3\theta - 3\cos \theta) }{8r^4} + \cdots \right]
\end{aligned}
$$
...
$$ {#eq-part2_legendre_expansion_x}

그렇다면 @eq-part2_electric_potential 에서 정해진 원점과 가능한 전하 분포 $\rho(\bf{r}')$ 에 대해 $\|\bf{r}\| > \|\bf{r}'\|$ 일 때,
...
$$
\phi(\bf{r}) = \dfrac{1}{4\pi\epsilon_0} \sum_{k=0}^\infty \dfrac{Q_k}{\|\bf{r}\|^{k+1}},\qquad \text{where }Q_k = \int_V \|\bf{r}'\|^k P_k(\cos\theta')\,\rho(\bf{r}')\,d^3\bf{r'}
$$ {#eq-part2_multiple_expansion}

이다. 

</br>

$\rho(\bf{r'})=q\delta(\bf{r}')$ 이라면, 즉 점전하라면 
