...
\end{aligned}
$$ {#eq-part2_electric_potential_dipole_1}


위의 경우는 총 전하량이 $0$ 이지만 양의 전하와 음의 전하가 분리되어 그 변위의 차이로 인해 $\|\bf{r}\|^{-2}$ 에 비례하는 전기장이 존재하는 경우이다. 이렇게 전하 분포가 균일하지 않으면 전체 전하의 합이 $0$ 이더라도 그 불균일성으로 인해 전위가, 따라서 전기장이 존재 할 수 있다. @eq-part2_electric_potential_dipole_1 에서 $1/r^2$ 에 비례하는 부분에 대해 전기 쌍극자 모멘트 $\bf{\mu}$ 를

$$
\bf{\mu}:=q\bf{d}
$$ {#eq-part2_electric_dipole_moment}

로 정의하면 전기 쌍극자 모멘트에 의한 전기 포텐셜 $\phi_d(\bf{r})$ 은 다음과 같다. 
$$
\phi_d (\bf{r})= \dfrac{1}{4\pi\epsilon_0} \dfrac{\bf{\mu\cdot r}}{r^3}
$$ {#eq-part2_electric_potential_dipole_2}
