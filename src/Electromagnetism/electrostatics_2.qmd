...
![균일한 전하 밀도를 가진 구에서의 전위차](../images/Electromagnetism/electrostatics/electrostatics_potential_uniform_sphere/electrostatics_potential_uniform_sphere.png){#fig-EM_electrosttics_potential_uniform_sphere width=450}
...
![전기 쌍극자](../images/Electromagnetism/electrostatics_2/electric_dipole.png){#fig-EM_electric_dipole width=200}
...
위의 경우는 총 전하량이 $0$ 이지만 양의 전하와 음의 전하가 분리되어 그 변위의 차이로 인해 $\|\bf{r}\|^{-2}$ 에 비례하는 전기장이 존재하는 경우이다. 이렇게 전하 분포가 균일하지 않으면 전체 전하의 합이 $0$ 이더라도 그 불균일성으로 인해 전위가, 따라서 전기장이 존재 할 수 있다. @eq-EM_electric_potential_dipole_1 에서 $1/r^2$ 에 비례하는 부분에 대해 전기 쌍극자 모멘트 $\bf{\mu}$ 를
...
$$ {#eq-EM_legendre_expansion_x}

그렇다면 @eq-EM_electric_potential 에서 정해진 원점과 가능한 전하 분포 $\rho(\bf{r}')$ 에 대해 $\|\bf{r}\| > \|\bf{r}'\|$ 일 때,
