...
### 극성 분자의 정렬 {#sec-ED_ES_alignement_of_polar_molecule}

H<sub>2</sub>O 와 같은 내재된 영구적인 쌍극자 모멘트를 가진다(@fig-ED_ES_polar_molecule 의 오른쪽 그림). 이런 문자를 **극성 분자 (polar molecule)** 이라고 한다. 물의 쌍극자 모멘트는 다른 분자에 비해 비정상적으로 큰데 $6.1 \times 10^{−30} \text{C}\cdot\text{m}$ 이다. 이러한 극성 분자에 전기장 $\bf{E}$ 를 가한다고 생각해보자.


![CO<sub>2</sub> 분자의 배치](figures/electrostatics/polar_molecule/polar_molecule.png){#fig-ED_ES_polar_molecule width=450}

전기장 $\bf{E}$ 가 균일하다면 총 전하가 $0$ 이므로 전하에 가히지는 힘은 $0$ 이다. 그러나 토크 $\bf{N}$ 을 계산해보면

$$
\bf{N} = (\bf{r}^++\times \bf{F}^+) + (\bf{r}^-\times \bf{F}^-)= q\bf{d}\times \bf{E}
$$

이며, 쌍극자 모멘트 $\bf{p}=q\bf{d}$ 를 생각하면 균일한 전기장에 놓인 쌍극자 모멘트에 가해지는 토크는 다음과 같다는 것을 알 수 있다.

$$
\bf{N} = \bf{p}\times \bf{E}.
$$ {#eq-ED_ES_torque_on_dipole_by_uniform_electric_field}

가해지는 전기장이 균일하지 않다면 $\|\bf{F}^+\| \ne \|\bf{F}^-\|$ 일 수 있으며 이 경우 극성 분자는

$$
\bf{F}=\bf{F}^++\bf{F}^-= q(\Delta \bf{E})
$$

의 힘을 받는다. 여기서 $\Delta \bf{E}$ 는 전하의 위치에서의 전기장의 차이이다. 일반적으로 쌍극자에서의 두 전하의 거리는 매우 짧기 때문에

$$
\Delta \bf{E} = (\bf{d\cdot} \nabla)\bf{E}
$$
...
$$
\bf{F}=(\bf{p\cdot}\nabla) \bf{E}
$$ {#eq-ED_ES_force_on_dipole_moment_by_nonuniform_electric_field}
...
::: {#exm-ED_ES_energy_of_dipole_in_field}

#### 전기장 안의 쌍극자의 에너지

전기장 $\bf{E}$ 가 가해진 쌍극자 $\bf{p}$ 의 에너지 $U$ 는

$$
U = -\bf{p \cdot E}
$$ {#eq-ED_ES_energy_of_dipole_in_field}

로 쓸 수 있다. 전기장 내의 쌍극자는 힘을 받지 않으며 토크만 맏는다. 토크 $\bf{N}=\bf{p}\times \bf{E}$ 이며 따라서 $N=pE\sin \theta$ 이다. 토크는 항상 $\bf{p}$ 와 $\bf{E}$ 가 평행하도록 작용하며 따라서 
...
### 편극 벡터 $\bf{P}$ {#sec-ED_ES_polarizaiton_vector_P}

앞서 우리는 유전 물질을 전기장에 배치하면,

1. 중성 원자(또는 비극성 분자)로 구성되었을 경우 전기장이 각 원자(분자)에 작은 쌍극자 모멘트를 유도하거나
2. 물질이 극성 분자로 이루어져 있다면, 영구 쌍극자에 의한 토크로 인해 그 쌍극자 모멘트가 전기장 방향으로 정렬되는 경향이 생간다.이 과정에서 무작위적인 열운동과 경쟁하므로 정렬은 완전하지 않다. 

어쨌든 두가지 경우 모두 대량의 쌍극자 모멘트가 발생하며 이 때 단위 부피당 쌍극자 모멘트를 **편극 (polarizaiotn)** 이라고 하고 $\bf{P}$ 로 표기한다.

우리는 쌍극자에 의한 정전포텐셜이

$$
\Phi(\bf{r})= \dfrac{1}{4\pi\epsilon_0}\dfrac{\bf{p\cdot}(\bf{r}-\bf{r}')}{\|\bf{r}-\bf{r}'\|^3}
$$

임을 안다. 그렇다면 우리가 위치마다의 편극 벡터 $\bf{P}(\bf{r}')$ 을 안다면 이 유전물질에 의한 정전포텐셜은

$$
\Phi (\bf{r})= \dfrac{1}{4\pi\epsilon_0} \int_V \dfrac{\bf{P}(\bf{r}')\bf{\cdot} (\bf{r}-\bf{r}')}{\|\bf{r}-\bf{r}'\|^3}d^3\bf{r}'
$$ {#eq-ED_ES_electrostatic_potential_by_polarizaiton}

이다. 유전물질이 차지한 부피 $V$ 와 그 표면 $S=\partial V$ 를 생각하자. 항등식

$$
\nabla' \left(\dfrac{1}{\|\bf{r}-\bf{r}'\|}\right) = \dfrac{\bf{r}-\bf{r}'}{\|\bf{r}-\bf{r}'\|^3}
$$
...
$$
\begin{aligned}
\Phi (\bf{r}) &= \dfrac{1}{4\pi\epsilon_0} \int_V \bf{P}(\bf{r}')\bf{\cdot}\nabla'\left(\dfrac{1}{\|\bf{r}-\bf{r}'\|}\right)\,d^3\bf{r} \\[0.3em]
&= \dfrac{1}{4\pi\epsilon_0} \left[\int_V \nabla'\bf{\cdot}\left(\dfrac{\bf{P(\bf{r}')}}{\|\bf{r}-\bf{r}'\|}\right)\,d^3\bf{r} - \int_V \dfrac{\nabla' \bf{\cdot P}(\bf{r}')}{\|\bf{r}-\bf{r}'\|} \,d^3\bf{r} \right] \\[0.3em]
&= \dfrac{1}{4\pi\epsilon_0}\int_S \dfrac{\bf{P\cdot}\hat{\bf{n}}}{\|\bf{r}-\bf{r}'\|}da  + \dfrac{1}{4\pi\epsilon_0}\int_V \dfrac{\left(-\nabla\bf{\cdot P}\right)}{\|\bf{r}-\bf{r}'\|}\,d^3\bf{r}.
\end{aligned}
$$
...
$$
\boxed{\sigma_b = \bf{P \cdot}\hat{\bf{n}}},\qquad \boxed{\rho_b= - \nabla \bf{\cdot P}}.
$$ {#eq-ED_ES_bounded_charged_density}


</br><div class="border" style="background-color:#F2F4F4  ;padding:5px;">

::: {#exm-ED_ES_potential_by_uniformly_polarized_sphere}

#### 균일하게 편극된 구에 의한 정전포텐셜과 전기장

아래 그림과 같이 균일하게 편극된 구에 의한 정전포텐셜과 전기장을 구해보자.

![균일한 $z$ 방향으로의 편극](figures/electrostatics/uniformly_polarized_sphere/uniformly_polarized_sphere.png){#fig-ED_ES_uniformly_polarized_sphere width=150}



우선 편극이 균일하므로 @eq-ED_ES_bounded_charged_density 에 대해

$$
\sigma_b = P\cos\theta,\qquad \rho_b = 0
$$

이다. 이제 $r<R$ 인 경우와 $r>R$ 인 경우로 나눠 보자. 
@eq-ED_MT_spherical_haromics_expansion_of_inverse_vector_difference 를 사용한다.

($1$) $r<R$ 인 경우

$$
\begin{aligned}
\Phi_{\text{in}}(\bf{r}) &= \dfrac{1}{4\pi\epsilon_0}\sum_{l=0}^\infty \sum_{m=-l}^{l}\dfrac{4\pi}{2l+1}\dfrac{r^l}{R^{l-1}}\int P\cos\theta' {Y_l^m}^\ast (\theta',\,\varphi')Y_l^m(\theta,\,\varphi) d\Omega'  \\[0.3em]
&= \dfrac{P}{\epsilon_0}\sum_{l=0}^\infty\sum_{m=-l}^{l}\dfrac{1}{2l+1}\dfrac{r^n}{R^{n-1}}Y_l^m (\theta,\,\varphi) \int \cos\theta' {Y_l^m}^\ast (\theta',\,\varphi') d\Omega' \\[0.3em]
&=\dfrac{P}{\epsilon_0}\sum_{l=0}^\infty\sum_{m=-l}^{l}\dfrac{1}{2l+1}\dfrac{r^l}{R^{l-1}}Y_l^m (\theta,\,\varphi) \sqrt{\dfrac{4\pi}{3}}\underbrace{\int Y_1^0(\theta',\,\varphi') {Y_l^m}^\ast (\theta',\,\varphi') d\Omega' }_{=\delta_{l1}\delta_{m0}}\\[0.3em] 
&=\dfrac{Pr}{\epsilon_0}\sqrt{\dfrac{4\pi}{3}} \dfrac{1}{3} Y_1^0 (\theta,\,\varphi) = \dfrac{Pr\cos\theta}{3\epsilon_0}.
\end{aligned}
$$
...
$$
\bf{E}_\text{in} = - \nabla \Phi_\text{in}= - \dfrac{\bf{P}}{3\epsilon_0}
$$
