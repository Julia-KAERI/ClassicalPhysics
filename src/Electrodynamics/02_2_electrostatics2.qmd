...
![같은 중심을 가진 두 구 껍질에 의한 축전지](figures/electrostatics/concentric_sphere_capacitor/figure.png){#fig-ED_concentric_sphere_capacitor width=150}
...
### 도체에서의 경계값 문제 {#sec-ED_boundary_value_problem_in_conductors}
...
::: {#thm-ED_uniqueness_theorem_for_dirichlet_and_neumann_boundary_condition}
...
$$
\int_V \nabla \bf{\cdot} (f\nabla f)\,d^3\bf{r} = \sum_i\int_{S_i} f\nabla f\,d\bf{S} = \sum_i \int_{S_i}  f (\nabla f \cdot \hat{\bf{n}})dS.
$$
...
@thm-ED_uniqueness_theorem_for_dirichlet_and_neumann_boundary_condition 는 일단 경계조건을 만족하는 포텐셜을 구하기만 한다면 그 포텐셜은 유일하다는 것을 의미한다. 이것을 이용하는 방법중의 하나가 **거울영상볍 (method of images)** 이다. 관심 있는 영역에 대한 포텐셜을 구할 때 이 영역이 아닌 다른 영역에 가상의 전하를 배치하여 경계조건을 만족하게 하여 포텐셜을 구하는 방법이다. 일단 가장 간단한 예시를 보자.

</br><div class="border" style="background-color:#F2F4F4  ;padding:5px;">

::: {#exm-ED_image_method_plane}

#### 도체 평면 근처의 전하

::: {#fig-ED_image_method_plane layout-ncol=2}

![$+q$ 의 전하가 위치했을 때 $-q$ 거울 전하의 위치](figures/electrostatics/image_method_plaine/fig1.png){#fig-ED_image_method_plane_config}

![도체 평면과 전하에 의한 전기장](figures/electrostatics/image_method_plaine/fig2.png){#fig-ED_image_method_plane_field}
