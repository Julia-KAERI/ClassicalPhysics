...
일반화된 좌표 $q_j$ 에 대해 $q_j \to q_j + \delta q_j$ 변환이 시스템의 전체 입자를 특정한 방향으로 병진 이동(translation) 시키는 변환이라고 하자. 예를 들면 데카르트 좌표계에서의 시스템의 질량중심. 운동에너지 $T$ 는 원점에 영향을 받지 않으므로, $\partial T/\partial q_j=0$ 일 것이다. 또한 보존계이며 포텐셜 에너지 $V$ 가 속도에 의존하지 않는다고 가정하자. 이 때의 라그랑쥬 방정식은 다음과 같다.
...
$$
Q_j = \sum_i \bf{F}_i \bf{\cdot} \dfrac{\partial \bf{r}_i}{\partial q_j}= \sum_i \bf{F\cdot n}, \quad \text{where }\bf{n} := \dfrac{\partial \bf{r}_i}{\partial q_j}
$$

여기서 $\bf{n}$ 은 병진 벡터 이므로 개개의 입자들의 위치 벡터 $\bf{r}_i$ 에 독립적임에 유의하자. 따라서,

$$
Q_j = -\dfrac{\partial V}{\partial q_j} = \sum_i \bf{F}_i \cdot \dfrac{\partial \bf{r}_i}{\partial q_j}= \bf{n} \cdot \sum_i \bf{F}_i = \bf{n\cdot F}
$$
...
$$
\begin{aligned}
p_j &= \dfrac{\partial T}{\partial \dot{q}_j} = \dfrac{\partial}{\partial \dot{q}_j}\left( \dfrac{1}{2} \sum_i m_i {\dot{\bf{r}}_i}^2 \right) = \sum_i m_i \bf{v}_i \cdot \dfrac{\partial \dot{\bf{r}}_i}{\partial \dot{q}_j} =\sum_i m_i \bf{v}_i \cdot \dfrac{\partial \bf{r}_i}{\partial q_j} \\[0.3em]
&= \bf{n \cdot} \left( \sum_i m_i \bf{v}_i \right)
\end{aligned}
$$
...
마찬가지로 $q_j \mapsto q_j+dq_j$ 변환이 어떤 축에 대한 전체 입자 각각의 위치 벡터 $\bf{r}_i$ 의 미소회전(infinitesimal rotation) 변환이며 $\hat{\bf{n}}$ 을 그 회전축의 단위벡터라고 하자 (@fig-CM_variation_rotational_symmetry). 그렇다면, $|d\bf{r}_i| = r_i \sin \theta \, dq_j$ 이다. 이로부터, $\bf{r}_i$의 $q_j$ 에 대한 편미분은 $\bf{r}_i$ 와 $\bf{n}$ 모두와 직교할 것이므로, 아래와 같은 꼴을 가질 것임을 알 수 있다.
