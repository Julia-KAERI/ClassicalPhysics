...
가 stationary valure 가 되도록 하는 경로 $\bf{x}(t)$ 를 찾는 것이다. 이것을 **변분법 (calculus of variation)** 이라고 한다.
...
여기서 $\alpha$ 는 infinitesimal deviation을 위한 parameter이며 $\eta(x)$ 는 임의의 경로인데, 경로의 시작과 끝이 정해졌다는 가정 하에서 생각하므로 $\eta(x_1)=\eta(x_2)=0$ 이다. 또한 우리는 $\eta(x)$ 가 well-behaved function 이라고 가정한다. 즉 $\eta(x)$ 는 $[x_1,\,x_2]$ 구간에서 연속이며 [non-singular](https://en.wikipedia.org/wiki/Singular_function) 하다고 생각한다. 
...
따라서 stationary condition 인 @eq-CM_stationary_condition 은 다음과 같게 된다.
...
따라서 우리는 @eq-CM_stationary_condition_2 로부터 $J$ 가 stationary value를 갖기 위해서는 다음을 만족해야 함을 알게 되었다. 이를 1차원에서의 **오일러-라그랑쥬 방정식(Euler-Lagrange equation)** 이라고 한다.

$$
\dfrac{ \partial f}{\partial y} - \dfrac{d}{dx} \left(\dfrac{\partial f}{\partial \dot{y}}\right)=0\;.
$$ {#eq-CM_euler_lagrange_equation_1d}
 
Differential quantities $\delta y,\, \delta J$ 를 $\delta y \equiv \left( \dfrac{\partial y}{\partial \alpha}\right)_{0} d\alpha$, $\delta J \equiv \left(\dfrac{dJ}{d\alpha}\right)_0 d\alpha$ 로 정의하자. 아래첨자 $0$ 은 $\alpha=0$ 을 의미한다. 이 정의를 이용하면 @eq-CM_stationary_condition_2 는 다음과 같이 쓸 수 있다.
...
### 구성공간 와 시스템의 동역학 {#sec-CM_configuration_space_and_system_dynamics}

달랑베르 원리를 통해 시스템의 특정 순간의 상태에서의 가상 변위(virtual dispacement)를 이용하여 라그랑쥬 방정식을 유도하였다. 라그랑쥬 방정식은 전혀 다른 방법, 즉 특정 시간 구간 $[t_1,\,t_2]$ 에 대한 일반화된 좌표의 구성공간 에서의 경로와 이 경로에 대한 virtual variation을 이용하여 유도될 수 있다. 이 때 시간 $t$ 는 변수가 아닌 매개변수로 고려되며 구성공간 상의 하나의 점은 하나 이상의 $t$ 값에 해당 할 수 있다.

</br>

### 해밀턴의 원리 {#sec-CM_least_action_theorem}
...
해밀턴의 원리는 monogenic system의 운동을 기술한다. 이 때 일반화된 포텐셜은 포텐셜이 $\{q_i\} = \bf{q},\,\{\dot{q}_i\}=\dot{\bf{q}},\, t$ 의 함수인 것을 말한다. Monogenic system의 포텐셜이 위치만의 함수 일 때, 이 시스템은 보존적이다. 
...
이 stationary value를 갖도록 하는 경로를 따른다. 이 선적분 $S$ 의 값을 **action** 이라고 한다. 다시 말하자면  $t_1$ 에서의 $\bf{q}_1 = \bf{q}(t_1)$ 에서 $t_2$ 에서의 $\bf{q}_2 =\bf{q}(t_2)$ 사이의 모든 가능한 경로중 실제 자연에 의해 선택되는 경로는 @eq-CM_definition_of_action 이 stationary value를 갖도록 하는 경로이다.
:::
...
우리는 변분법에서 라그랑쥬 방정식을 유도할 때 모든 $\bf{q}$ 의 성분이 각각 독립임을 가정하였다. 이것은 구성공간에서의 holonomic 조건과 동치이다. 즉 holonomic 조건에서 해밀톤의 원리를 이용하여 달랑베르 정리와 같은 결과를 얻었다.
...
- 해밀턴의 원리를 이용해서든 달랑베르 원리(D'Alembert's principle)을 이용해서든 holonomic 구속 조건에서 라그랑쥬 방정식을 얻을 때, holonomic 구속 조건은 맨 마지막에 가서 변수들의 variation 이 독립적이라는 것을 가정 할 때 나온다. Holonomic 구속 조건이 부여된 시스템에서는 당장의 일반화된 좌표가 독립적이지 않더라도 $f(q_1,\ldots,\,q_n,\,t)=0$ 꼴의 equation of constraints 를 통해 독립적인 일반화된 좌표계를 구성 할 수 있는데, nonholonomic constraints system 에서는 일반적으로 이것이 불가능하다. 
...
이제 *Lagrange multiplier method* 를 사용하자. 위의 @eq-CM_nonholonomic_displacement 에서 모든 $l=1,\,2,\ldots,\,m$ 에 대해
...
Lagrange multiplier 방법을 이용하기 위해 @eq-CM_lagrange_multiplier 를 다음과 같이 쓸 수 있다.
...
여기서 $\{ \delta q_k\}$ 는 여전히 dependent 하며 $m$ 개의 관계- @eq-CM_nonholonomic_displacement -로 연결되어 있다. 따라서 $n-m$ 개의 $\delta q_k$ 들이 독립적이 되도록 선택 될 수 있다. 하지만 아직 $m$ 개의 $\lambda_l$ 값을 우리는 모른다. $\lambda_l$ 이 다음을 만족하도록 선택된다고 하자.
...
이것은 $1$ 부터 $n$ 까지의 변수 $\{q_k\}$ 중 마지막 $m$ 개에 대한 equation of motion 이다.  그렇다면 앞의 $n-m$ 개의 $\{q_k\}$ 에 대해 @eq-CM_lagrange_multiplier_2 를 다음과 같이 쓸 수 있다.
...
System에 대한 constraints를 제거하고 대신에 applied external forces $Q'_k$ 를 system의 motion이 변화하지 않도록 놓자. 명백하게 applied external forces는 forces of constraints 이다. @eq-CM_lagrange_multiplier_4 에서 $Q'_k = \sum\limits_l \lambda_l \, a_{lk}$ 로 놓으면,
...
### Phase space {#sec-CM_variational_phase_space}

우리는 $n$ 개의 일반화된 좌표 $q_1,\ldots,\,q_n$ 으로 구성되는 공간을 **구성공간(configuration space)** 라고 불렀다. 이와 달리 일반화된 좌표 $q_1,\ldots,\,q_n$ 과 그 정준 모멘트 $p_1,\ldots,\,p_n$ 으로 구성되는 공간을 **위상공간(phase space)** 라고 한다. 
...
#### Canonical momentum
Monogenic system이며 포텐셜이 위치에만 의존하면 $\partial L /\partial \dot{x}_j = m\ddot{x}_j$ 이며 우리가 아는 momentum이다. 이것을 일반화하여 $q_j$ 에 대한 **정준 운동량(canonical momentum or conjugate momentum)** $p_j$ 를 다음과 같이 정의한다.
...
여기서 $q_j$ 는 데카르트 좌표계의 좌표가 아닌 일반화된 좌표계의 좌표이므로 $p_j$ 는 선형 운동량의 차원이 아닌 다른 차원을 가질 수 있다. 또한 속도-의존적 포텐셜을 가진 시스템 에서는 소위 역학적 운동량(mechanical momentum)과 다른 값을 가진다. 예를 들어 전자기장 에서의 입자계의 라그랑지안은 다음과 같이 주어진다. (아래의 $q_i$ 는 $i$ 번째 입자의 전하(charge) 이다.)

$$
L = \sum_i \dfrac{1}{2} m_i {\dot{r}_i}^2 - \sum_i q_i \phi(x_i) + \sum_i {q_i} \bf{A}(x_i) \cdot \dot{\bf{r}}_i\;.
$$

이 때의 일반화된 운동량 $\bf{p}$ 가 mehcanical momentum과 다름은 쉽게 보일 수 있다.

</br>

### 순환 좌표 {#sec-CM_cyclic_coordinate_lagrangian}

일반화된 좌표계에서 표현된 라그랑지안이 어떤 $q_i$ 에 대해 의존성이 없을 경우, 즉 $\dfrac{\partial L}{\partial q_i}=0$ 일 경우 $\dfrac{\partial L}{\partial \dot{q_j}}=p_j = \text{const.}$ 이다. 이때의 $p_j$ 를 **순환 좌표(cyclic coordinate)** 라고 한다. 즉 순환좌표에 $q_j$ 에 대한 정준 운동량 $p_j$ 는 보존된다. 이렇게 보존되는 $p_j$ 는 라그랑지안에서 제일적분이다.
...
### 병진 대칭 (Translational Symmetry) {#sec-CM_variation_translational_symmetry}
...
일반화된 좌표계 $q_j$ 에 대해 $\bf{r}_i(q_j+dq_j) -\bf{r}_i (q_j)$ 는 어떤 특정한 방향으로의 병진 이동(translation)을 뜻한다고 생각해보자. 예를 들면 데카르트 좌표계에서의 시스템의 질량중심. 운동에너지 $T$ 는 원점(origin)에 영향을 받지 않으므로, $\partial T/\partial q_j=0$ 일 것이다. 또한 보존계 이며 포텐셜 에너지 $V$가 velocity-indepedent 라고 가정하자. 이 때의 라그랑쥬 방정식은 다음과 같다.
...
이다. 즉, $p_j$ 는 시스템의 총 선형 운동량의 $j$-번째 성분이다. 그렇다면 $q_j$ 가 순환좌표인 경우, $Q_j=0$ 이므로 $\dot{p}_j=0$ 이다. 즉 $p_j=\text{const.}$ 이다.
...
$q_j$ 가 cyclic 일 경우 $Q_j=0$ 이며, 따라서 $\bf{n} \cdot \bf{L}=\text{constant}$ 이다. 즉 각운동량 보존.
