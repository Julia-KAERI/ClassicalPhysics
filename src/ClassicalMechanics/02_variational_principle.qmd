...
![1차원 경로](../images/classical%20mechanics/path1d/path1d.png){#fig-1d_path_variational width=250}
...
따라서 stationary condition 식 @eq-CM_stationary_condition 은 다음과 같게 된다.
...
::: {#exm-CM_minima_volume_of_a_body_of_revolution}
...
::: {#exm-CM_classical_brachistochrone_problem}

#### The brachistochrone problem

![Brachistochrone](../images/classical%20mechanics/brachistochrone/brachistochrone.png){width=30%}
...
이 stationary value를 갖도록 하는 경로를 따른다. 이 선적분의 값을 **action** 이라고 한다. 다시 말하자면  $t_1$ 에서의 위치 $\boldsymbol{x}_1$ 에서 $t_2$ 에서의 위치 $\boldsymbol{x}_2$ 사이의 모든 가능한 경로중 실제 자연에 의해 선택되는 경로는 @eq-CM_definition_of_action 이 stationary value를 갖도록 하는 경로이다.
:::
...
우리는 @eq-CM_euler_lagrange_for_muntidimensional_case 을 통해 라그랑쥬 방정식을 얻을 수 있다.
...
여기서 $l$ 은 이러한 nonholonomic equations of constraints에 대한 index 이다. $m$개의 이러한 방정식이 존재한다고 생각하자. 그리고 $a_{lk}$ 와 $a_{lt}$ 는 $q$ 와 time의 함수일 수 있다는 것을 유념하자. 변분경로 혹은 변분경로를 구성하는 변위는 @eq-CM_nonholonomic_constraints 의 제약 조건을 충족해야 한다고 예상된다. 그러나 @eq-CM_nonholonomic_constraints 가 적분 가능(integrable) 하지 않으면 이러한 다양한 경로를 구성할 수 없다는 것이 증명 되었으며, 이 경우 제약 조건은 실제로는 homolomic 이다. 그럼에도 불구하고 정확한 운동 방정식으로 이어지는 변분 원리는 virtual displacement 에 의해 실제 운동으로부터 다양한 경로가 구성될 때 얻을 수 있다.
...
![Translational Symmetry](../images/classical%20mechanics/translational_symmetry/translational_symmetry.png){#fig-part1_translational_symmetry width=50%}
...
우리는 이것이 총 선형 운동량에 대한 운동방정식임을 안다. $Q_j$ 는 total force $\boldsymbol{F}$ 의 $q_j$ 의 translation 방향 성분으로 아래와 같이 표현 할 수 있다.(@eq-CM_generalized_force)
...
이다. 이제 conjugate momentum of $q_j$ 를 보면, (@eq-CM_dlambert_1 를 이용하였다.)
...
![Rotational Symmetry](../images/classical%20mechanics/rotation_of_vector/rotation_of_vector.png){#fig-rotsym width=350}
