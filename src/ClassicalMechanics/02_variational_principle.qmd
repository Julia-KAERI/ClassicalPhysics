...
미분방정식으로 기술되는 물리계에서 그 경로 전체에 대해 어떤 위상의 좌표 즉 일반화된 좌표와 운동량, 그리고 시간에 대한 함수 $f = f(\bf{q},\,\dot{\bf{q}},\,t)$ 에 대해 $\dfrac{df}{dt}=0$ 일 때 $f$ 를 이 계의 **제일적분(first integral)** 라고 한다.
...
$$
\dfrac{d f(\bf{q},\, \dot{\bf{q}},\,t)}{dt} = \sum_i \left(\dfrac{\partial f}{\partial t}\dot{q}_i + \dfrac{\partial f}{\partial \dot{q}_i}\ddot{q}_i \right) + \dfrac{\partial f}{\partial t} = 0.
$$
