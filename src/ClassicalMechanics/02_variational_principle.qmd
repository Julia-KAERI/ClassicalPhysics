...
### 범함수와 변분법 {#sec-CM_functional_and_calculus_of_variation}

수학적으로 어떤 함수의 집합 $X$ 에 대해 $X \mapsto \mathbb{R}$ 인 함수를 **범함수(functional)** 라고 한다. **변분법 (calculus of variation)** 은 어떤 경로 의존적인 함수 $f(\bf{x},\, \dot{\bf{x}},\,t)$ 에 대해

$$
J = \int_C f(\bf{x},\, \dot{\bf{x}},\,t)\,dt
$$

가 정류값(stationary value)$^{1}$[변분법은 범함수의 극댓값, 극솟값을 연구하는데, 이를 합쳐서 정류값이라 한다.]{.aside} 이 되도록 하는 경로 $\bf{x}(t)$ 를 찾는 방법이다.
...
- 해밀턴의 원리를 이용해서든 달랑베르 원리(D'Alembert's principle)을 이용해서든 holonomic 구속 조건에서 라그랑쥬 방정식을 얻을 때, holonomic 구속 조건은 맨 마지막에 가서 변수들의 variation 이 독립적이라는 것을 가정 할 때 나온다. Holonomic 구속 조건이 부여된 시스템에서는 당장의 일반화된 좌표가 독립적이지 않더라도 $f(q_1,\ldots,\,q_n,\,t)=0$ 꼴의 구속 조건에 대한 방정식을 통해 독립적인 일반화된 기준틀을 구성 할 수 있는데, nonholonomic 구속조건에서는 일반적으로 이것이 불가능하다. 

- 변동 경로는 $y_i (x,\,\alpha) = y_i (x,\,0)+\alpha \eta_i (x)$ 형태로 표현된다. 그리고 $\delta y_i$ 는 실제 경로와 이웃의 가상 경로의 가상 변위를 의미한다. 독립적인 일반화된 기준틀 조건에서는 마지막 최종 변경 후의 점(points) 만 중요하지만 변수들이 독립적이지 않으면 변위가 구속조건을 따르며 변화했는지 여부가 매우 중요하다. 
...
앞서 변분 역학을 non-holonomic 구속 조건을 포함하도록 확장했지만 변분 역학의 가장 큰 장점은 monogenic 조건과 holonomic 구속 조건에서 독립적인 일반화된 좌표를 사용할 때 드러난다. 변분 역학은 간결하고 우아할 뿐만 아니라 특정한 일반화된 기준틀에 구애되지 않고 운동에너지와 포텐셜 에너지라는 물리량만을 이용한다는 장점이 있다. 여기서 나온 식들은 좌표의 변환에 대해 불변이다. 즉 $\bf{q} = (x,\,y,\,z)$ 이든 $\bf{q} = (r,\,\theta,\, \varphi)$ 이든 오일러-라그랑쥬 방정식의 형태는 동일하다.
...
$n$-자유도 시스템은 시간에 대한 $n$ 개의 2차 미분방정식으로 표현되며, $2n$ 개의 적분상수를 필요로 한다. 이들은 대개 초기조건으로부터 얻어지며 보통 시간 $t=t_0$ 에 대해 각각 $n$ 개의 $q_j(t_0)$ 와 $\dot{q}_j(t_0)$ 값이다. 그러나 상당수의 경우 (아마도 대부분의 경우) 이것들이 정확하게 풀리지 않는다. 하지만 완전한 해가 얻어질 수 없더라도 중요한 물리를 얻을 수 있는 경우가 있다. 이 때 중요한 것이 소위 제일적분이다.
...
미분방정식으로 기술되는 물리계에서 그 경로 전체에 대해 어떤 위상의 좌표 즉 일반화된 좌표와 운동량, 그리고 시간에 대한 함수 $f = f(\bf{q},\,\dot{\bf{q}},\,t)$ 에 대해 $df/dt=0$ 일 때 이런 함수들을 이 계의 **운동상수 (constant of motion)** 라고 한다. 운동 상수 가운데 시간에 대한 명시적인 의존성이 없는 함수, 즉 $\partial g/\partial t=0$ 인 운동 상수 $g$ 를 **제일적분(first integrals)** 혹은 **운동적분 (integrals of motion)** 이라고 한다.
...
::: {#exm-CM_constant_of_motion_but_not_first_integral} 

1차원 에서 속도 $v$ 로 움직이는 입자에 대해 $C(x,\,t) = x-vt$ 는 운동상수이지만 제일적분은 아니다.
...
또 하나 흥미로운 경우는 일반화된 기준틀에서 표현된 라그랑지안이 어떤 일반화된 좌표 $q_i$ 에 대해 의존성이 없을 경우, 즉 $\dfrac{\partial L}{\partial q_i}=0$ 일 경우이다. 이 때 $\dfrac{\partial L}{\partial \dot{q_j}}=p_j = \text{const.}$ 이며 이때의 $p_j$ 를 **순환 좌표(cyclic coordinate)** 라고 한다. 즉 순환좌표에 $q_j$ 에 대한 정준 운동량 $p_j$ 는 보존된다. 이렇게 보존되는 $p_j$ 는 라그랑지안에서 제일적분이다.

</br>

이렇게 일반화된 운동량 보존은 우리가 알고 있는 선형 운동량 보존이나 각운동량 보존보다 일반적이다. 예를 들어 전자기장에서의 하전 입자의 운동에서 보존되는것은 전자기장에서의 일반화된 운동량이다.
...
일반화된 좌표 $q_j$ 에 대해 $q_j \to q_j + \delta q_j$ 변환이 시스템의 전체 입자를 특정한 방향으로 병진 이동(translation) 시키는 변환이라고 하자. 즉 시스템의 모든 입자에 대해

$$
\dfrac{\partial \bf{r}_a}{\partial q_j} = \bf{n}
$$

이다. 운동에너지 $T$ 는 원점에 영향을 받지 않으므로, $\partial T/\partial q_j=0$ 일 것이다. 또한 보존계이며 포텐셜 에너지 $V$ 가 속도에 의존하지 않는다고 가정하자. 이 때의 라그랑쥬 방정식은 다음과 같다.

$$
\dot{p}_j \equiv \frac{d}{dt} \left( \frac{\partial T}{\partial \dot{q}_j} \right) = -\dfrac{\partial V}{\partial q_j} \equiv Q_j \;.
$$

우리는 이것이 총 선형 운동량에 대한 운동방정식임을 안다. $Q_j$ 는 총 힘 $\bf{F}$ 의 $q_j$ 에 의한 병진 방향 성분으로 아래와 같이 표현 할 수 있다.(@eq-CM_generalized_force)

$$
Q_j = \sum_a \bf{F}_a \bf{\cdot} \dfrac{\partial \bf{r}_a}{\partial q_j}= \sum_i \bf{F\cdot n}.
$$

여기서 $\bf{n}$ 은 병진 벡터 이므로 개개의 입자들의 위치 벡터 $\bf{r}_a$ 에 독립적임에 유의하자. 따라서,

$$
Q_j = -\dfrac{\partial V}{\partial q_j} = \sum_a \bf{F}_a \cdot \dfrac{\partial \bf{r}_a}{\partial q_j}= \bf{n} \cdot \sum_i \bf{F}_a = \bf{n\cdot F}
$$
...
$$
\begin{aligned}
p_j &= \dfrac{\partial T}{\partial \dot{q}_j} = \dfrac{\partial}{\partial \dot{q}_j}\left( \dfrac{1}{2} \sum_a m_a {\dot{\bf{r}}_a}^2 \right) = \sum_a m_a \bf{v}_a \cdot \dfrac{\partial \dot{\bf{r}}_a}{\partial \dot{q}_j} =\sum_i m_a \bf{v}_a \cdot \dfrac{\partial \bf{r}_a}{\partial q_j} \\[0.3em]
&= \bf{n \cdot} \left( \sum_a m_a \bf{v}_a \right)
\end{aligned}
$$
...
마찬가지로 $q_j \mapsto q_j+dq_j$ 변환이 어떤 축에 대한 전체 입자 각각의 위치 벡터 $\bf{r}_a$ 의 미소회전(infinitesimal rotation) 변환이며 $\hat{\bf{n}}$ 을 그 회전축의 단위벡터라고 하자 (@fig-CM_variation_rotational_symmetry). 그렇다면, $|d\bf{r}_a| = r_a \sin \theta \, dq_j$ 이다. 이로부터, $\bf{r}_a$의 $q_j$ 에 대한 편미분은 $\bf{r}_a$ 와 $\bf{n}$ 모두와 직교할 것이므로, 아래와 같은 꼴을 가질 것임을 알 수 있다.

$$
\dfrac{\partial \bf{r}_a}{\partial q_j} = \hat{\bf{n}}\times \bf{r}_a
$$
...
$$
Q_j = \sum_a \bf{F}_a \cdot (\hat{\bf{n}} \times \bf{r}_a) = \hat{\bf{n}} \cdot \left(\sum_a \bf{r}_a \times \bf{F}_a \right) = \hat{\bf{n}} \cdot \bf{N}
$$
...
$$
\begin{aligned}
p_j = \dfrac{\partial T}{\partial \dot{q}_j} = \sum_a m_a \bf{v}_a \cdot \dfrac{\partial \bf{r}_a}{\partial q_j} = \sum_a \bf{n} \cdot (\bf{r}_a \times m_i \bf{v}_a) = \bf{n}\cdot \sum_a \bf{L}_a = \bf{n} \cdot \bf{L}\;.
\end{aligned}
$$
...
::: {#exm-CM_bead_on_a_rotating_hoop} 

#### 회전하는 굴렁쇠에의 구슬

반지름 $a$ 인 굴렁쇠가 아래 @fig-CM_bead_on_a_rotating_hoop 와 같이 일정한 각속도 $\omega$ 로 회전한다고 하자. 회전하는 굴렁쇠 테 위에서 마찰 없이 움직이는 질량 $m$ 의 구슬을 생각하자.

![회전하는 굴러오시에서의 구슬](images/beads_on_a_rotating_hoop/beads.png){#fig-CM_bead_on_a_rotating_hoop width=300}

데카르트 좌표계에서 위치 $x,\,y,\,z$ 는 아래와 같다.

$$
x = a \sin \psi \cos \omega t,\qquad y = a \sin \psi \sin \omega t,\qquad z= a(1-\cos\psi).
$$

독립 변수는 $\psi$ 뿐이며 라그랑지안 $L(\psi)$ 는 다음과 같다.

$$
\begin{aligned}
L &= \dfrac{m}{2}(\dot{x}^2+\dot{y}^2+\dot{z}^2) - mgz \\[0.3em]
&=\dfrac{m}{2}[a\dot{\psi}^2 +  \omega^2 \sin^2 \psi]  + mag\cos \psi - mag.
\end{aligned}
$$

상수항 $-mag$ 는 라그랑지안에서 의미가 없으며 유표 포텐설 $V_\text{eff}$ 을 

$$
V_\text{eff}(\psi) = - mag \cos \psi - \dfrac{m\omega^2 \sin^2\psi}{2}
$$

라고 하면 라그링지안을 아래와 같이 쓸 수 있다.

$$
L = \dfrac{ma^2\dot{\psi}^2}{2} - V_\text{eff}(\psi)
$$

그렇다면 이 라그랑지안에 대한 오일러-라그랑쥬 방정식은 다음과 같다.

$$
\ddot{\psi} = -\dfrac{\partial V_\text{eff}}{\partial \psi} = -mag \sin \psi + m\omega^2 \sin\psi \cos \psi.
$$

아래 그림은 이 시스템의 유효 표텐셜 $V_\text{eff}$ 를 세가지 경우로 나누어 표현했다. 이로부터 굴렁쇠가 충분히 빠르게 회전한다면, 즉 $\omega^2 > g/a$ 라면 $\psi$ 에 대한 안정점은 $\psi\ne 0$ 이 아닌 위치에서 형성된다. 

![유효 포텐셜](images/beads_on_a_rotating_hoop/effective_potential.png){#fig-CM_effective_potential_for_bead_on_a_rotating_hoop width=500}
...
![Coplana double pendulum](images/landau_pr_5_1/figure.png){#fig-CM_landau_pr_5_1 width=250}

중력이 $-y$ 방향으로 작용할 때 위의 그람과 같은 이중 진자의 라그랑지안을 구하라.
...
첫번째 진자에 대한 운동에너지 $T_1$ 과 위치에너지 $U_1$ 은 아래와 같다.

$$
T_1=\dfrac{1}{2}m_1l_1^2 \dot{\phi_1}^2,\quad U_1 = -m_1gl_1 \cos \phi_1
$$

이제 두번째 진자의 위치를 $(x_2,\,y_2)$ 라고 하면

$$
x_2 = l_1 \sin \phi_1 + l_2 \sin \phi_2,\quad y_2 = -l_1\cos \phi_1 - l_2 \cos \phi_2
$$

이므로 두번째 진자의 운동에너지 $T_2$ 와 위치에너지 $U_2$ 는

$$
\begin{aligned}
T_2 &= \dfrac{1}{2}m_2 \left({\dot{x}_2}^2 + {\dot{y}_2}^2 \right)  \\[0.3em]
&=\dfrac{m_2}{2}\left({l_1}^2 {\dot{\phi}_1}^2 + {l_2}^2{\dot{\phi}_2}^2 + 2l_1l_2 \cos (\phi_1 - \phi_2) \dot{\phi}_1 \dot{\phi}_2\right)\\[0.3em]
U_2 &= - m_2 g(l_1 \cos \phi_1 + l_2 \cos \phi_2)
\end{aligned}
$$

이다. 라그랑지안 $L = T_1+T_2 - U_1 - U_2$ 이다.
...
![Coplana double pendulum](images/landau_pr_5_2/figure.png){#fig-CM_landau_pr_5_2 width=280}

위의 그림에서 질량  $m_1$ 은 $x$ 축상에서만 이동 가능하며 $m_2$ 는 $m_1$ 에 연결된 단순 진자이다. 라그랑지안을 구하라.
...
$T_1 = \dfrac{m_1 {\dot{x}_1}^2}{2}$, $U_1=0$ 이며

$$
x_2 = x_1 + l \sin \phi,\qquad y_2 =- l\cos \phi
$$
...
$$
\begin{aligned}
T_2 &= \dfrac{m_2}{2}\left[(\dot{x}_1+l\dot{\phi}\cos \phi)^2 + l^2 \dot{\phi}^2\sin^2\phi\right] \\[0.3em]
&=\dfrac{m_2}{2}\left[{\dot{x}_1}^2 + l^2\dot{\phi}^2 + 2\dot{x}_1l\dot{\phi}\cos\phi\right]\\[0.3em]
U_2 &= -m_2gl \cos \phi
\end{aligned}
$$
