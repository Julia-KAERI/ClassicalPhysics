...
붕괴에너지는 **환산 질량 (reduced mass)** $m:= \left(\dfrac{1}{m_1}+\dfrac{1}{m_2}\right)$ 에 대해 
...
질량 $M$ 인 입자가 자발적 붕괴를 통해 여러 입자로 붕괴된다고 하자. 붕괴된 입자가 가질 수 있는 에너지에는 상한이 존재한다. 이 붕괴된 입자들 가운데 질량 $m_1$ 인 입자를 생각하자. 원래 입자 $M$ 의 에너지를 $E_i$ 라 하고 $m_1$ 을 제외한 나머지 붕괴된 입자들의 에너지를 $E_i'$ 라고 하자. $m_1$ 의 에너지를 $E_1$ 이라고 하면
...
이다. $\hat{\bf{n}}_0$ 는 두 입자의 상호작용과, 충돌시 위치에 의해 결정된다. 이제 실험실 좌표계에서 생각하자. 질량중심의 속도 $\bf{V}= \dfrac{m_1\bf{v}_1 + m_2\bf{v}_2}{m_1+m_2}$ 에 대해 실험실 좌표계의 충돌 이후의 속도 $\bf{v}'_1,\, \bf{v}'_2$ 는
...
$$
\begin{aligned}
\bf{p}'_1 &= m_1 \bf{v}'_1 = m v \hat{\bf{n}}_0 + \dfrac{m_1(\bf{p}_1 + \bf{p_2})}{m_1+m_2}, \\
\bf{p}'_2 &= m_2 \bf{v}'_2 = -m v \hat{\bf{n}}_0 + \dfrac{m_2(\bf{p}_1 + \bf{p_2})}{m_1+m_2}
\end{aligned}
$$
...
$$
\overrightarrow{OC} = mv,\quad\overrightarrow{AO} = \dfrac{m_1}{m_1+m_2}(\bf{p}_1+ \bf{p}_2), \quad \overrightarrow{OB} = \dfrac{m_2}{m_1+m_2}(\bf{p}_1+ \bf{p}_2)
$$ 
...
많은 경우 충돌실험은 정지해 있는 한 입자에 대해 다른 입자를 충돌시킨다. 여기서 $m_2$ 입자가 충돌 전에 정지해 있다고 하자. 즉 $\bf{v}_2=\bf{0}$ 이며 $\bf{v}=\bf{v}_1$ 이다. $\overrightarrow{OB}=mv$ 이며, 따라서 $\overrightarrow{AB}= \bf{p}_1$ 이 된다 @fig-CM_elastic_scattering_angles_ss 는 $m_2$ 가 정지해 있을 때의 실험실 좌표계에서의 운동량에 관련된 도식이다.
...
$m_1=m_2$ 인 경우에는 $\bf{p}'_1 \cdot \bf{p}'_2 = 0$ 이 된다. 즉 $A,\,B,\,C$ 가 반경 $mv$ 인 원에 위치한다.
...
물리학에서 많이 이루어지는 산란 실험은 동일한 초기 속도 $\bf{v}_0$ 를 갖는 공간적으로 균일한 분포를 가진 다수의 입자를 입사시키며, 이 균일한 입자들이 $\bf{v}_0$ 와 수직한 방향으로 $A$ 만큼의 면적을 갖고 있다고 하자. $N$ 개의 입자가 입사되었을 때 산란각 $\Theta$ 와 $\Theta + d\Theta$ 사이로 산란되는 입자의 갯수를 $dN(\Theta)$ 이라고 하자. 입사 입자가 우리는 산란단면적 $d\sigma$ 를 다음과 같이 정의 할 수 있다.
