...
\begin{tikzpicture}[scale=1.2, rotate=0]
    ...
    \draw[thick] (-1,0) -- (A);
    \draw[thick] (0,-1) -- (B);
    \draw[-stealth, very thick] (O) -- (R1);
    \draw[-stealth, very thick] (O) -- (R2);
    \draw[-stealth, very thick, blue] (R1) -- (R2);
    \node [anchor=south east] at (1.25,1.5) {$\boldsymbol{r}_a(q_j)$};
    \node [anchor=north west] at (2,1.5) {$\boldsymbol{r}_a(q_j+dq_j)$};
