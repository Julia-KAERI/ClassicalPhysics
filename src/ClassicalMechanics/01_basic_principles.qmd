...
각운동량 $\boldsymbol{L}$ 은 다음과 같이 정의된다.
...
원점 $\mathcal{O}$ 로부터의 토크(torque) (혹은 moment of force) $\boldsymbol{N}$ 은 
...
여기서 작용-반작용이 $\boldsymbol{F}_{ji}=F_{ji}\hat{\boldsymbol{r}}_{ij}$ 일 경우를 생각하자. (이를 the strong law of action and reaction 라고 한다.) 이 경우 @eq-CM_derivative_of_total_angular_momentum 의 마지막 term은 아래와 같이 $0$ 이 된다.
...
의 조건을 얻게 되며, 위의 @eq-CM_principle_of_virtual_work 은 *the principle of virtual work* 이라 불린다. 이렇게 되면 각각의 $i$ 번째 입자에 대해 $\boldsymbol{F}^{(a)}_i$ 가 $0$ 인 조건이 아니게 되며, 따라서 $\delta \boldsymbol{r}_i$ 는 서로 독립적이지 않게 되고 위의 @eq-CM_principle_of_virtual_work 가 하나의 constraint 처럼 작용한다. @eq-CM_principle_of_virtual_work 는 constraint force $\boldsymbol{f}_i$ 를 포함하지 않는다는 측면에서 우리가 원하는 것이지만 단지 statics 만을 다룬다는 점에서 부족하다. 우리는 이것으로 부터 우리가 원하는 운동방정식을 얻고자 한다.
...
달랑베르 원리(@eq-CM_dalembert_principle) 은 $\sum\limits_i \dot{p}_i \cdot \delta \boldsymbol{r}_i = \sum\limits_i m_i \ddot{\boldsymbol{r}}_i \cdot \delta \boldsymbol{r}_i$ 로 쓸 수 있으며 
...
$$ {#eq-CM_dalembert_principle2}

데카르트 좌표계에서는 항상 $\dfrac{\partial T}{\partial q_j} = 0$ 이지만 일반화된 좌표계에서는 그렇지 않을 수 있음에 주의하라.

</br>

Holonomic constraints 조건에서는 독립적인 일반화된 좌표 $q_1,\ldots,\,q_n$ 을 찾을 수 있다. 그렇다면 @eq-CM_dalembert_principle2 는 각각의 $q_j$ 에 대해,

$$
\boxed{
\dfrac{d}{dt}\left( \dfrac{\partial T}{\partial \dot{q}_j}\right) - \dfrac{\partial T}{\partial q_j}= Q_j
}
$$ {#eq-CM_lagrange_from_dalembert_1}
...
그렇다면 @eq-CM_lagrange_from_dalembert_1 은 다음과 같다.

$$
\dfrac{d}{dt} \left( \dfrac{\partial T}{\partial \dot{q}_j}\right) - \dfrac{\partial (T-V)}{\partial q_j} = 0\;.
$$ {#eq-CM_lagrange_from_delembert_2}


@eq-CM_lagrange_from_delembert_2 형태의 운동방정식은 포텐셜 $V$ 가 꼭 보존계에만 제한되지 않는다. $V$ 가 시간에 대해 명시적인 함수가 아니기만 하면 이 시스템은 보존계이다. 


지금까지는 $V$ 가 generalized velocity에 의존하지 않는다고 가정하였다. 이 경우 @eq-CM_lagrange_from_delembert_2 는 다음과 같다.

$$
\dfrac{d}{dt} \left( \dfrac{\partial (T-V)}{\partial \dot{q}_j} \right) - \dfrac{\partial (T-V)}{\partial q_j} = 0
$$ {#eq-CM_lagrange_from_delembert_}
...
Lagrangian $L(q,\,\dot{q},\,t)$ 와 어떤 미분가능하며 $q,\,t$ 에만 의존하는 함수 $F(q,\,t)$ 에 대해, 

$$
L'(q,\,\dot{q},\,t) = L(q,\,\dot{q},\,t) + \dfrac{dF}{dt}
$$ 

라고 하면 $L'$ 에 대한 라그랑쥬 방정식은 $L$ 에 대한 라그랑쥬 방정식과 같다. 즉 라그랑쥬 방정식 어떤 System에 대한 Lagrangian은 unique 하지 않다. 
...
을 얻는다. 즉 @eq-CM_electromagnetic_potential 로부터 얻은 generalized force 는 로런츠 힘(Lorentz force) 이다.
...
라그랑쥬 방정식 (@eq-CM_lagrange_from_dalembert_1) 은 다음과 같이 변형됨을 보여라. 아래의 방정식을 라그랑쥬 방정식의 *Nielson 형식* 이라고 한다.
