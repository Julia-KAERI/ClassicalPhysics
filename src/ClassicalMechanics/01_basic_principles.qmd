...
다수의 입자로 이루어진 입자계에서 우리는 계를 이루는 입자들 사이의 힘과 입자계 외부의 원인에 의해 입자계의 입자에 가해지는 힘을 모두 고려해야 한다. $N$ 개의 입자로 이루어진 계를 $N$-입자계라고 하자. 입자계에서 $b$-번째 입자가 $a$-번째 입자에 가하는 힘을 $\bf{F}_{ba}$ 라 하고 시스템 외부에서 $a$-번째 입자에 가하는 힘을 $\bf{F}^{(e)}_{a}$ 라 하면 $a$ 번째 입자에 가해지는 힘은 $\bf{F}_{ba} + \bf{F}^{(e)}_a$ 이며, 따라서 운동방정식은 다음과 같다.

$$
\dfrac{d\bf{p}_a}{dt} = \frac{d}{dt} (m_a\bf{v}_a) = \sum_b \bf{F}_{ba} + \bf{F}^{(e)}_a.
$$ {#eq-CM_equation_of_motion_in_particle_system}

제 3 운동법칙에 따라 모든 $a,\,b$ 에 대해 $\bf{F}_{ab} = - \bf{F}_{ba}$ 일 경우 모든 입자에 대해 더하면 다음과 같다. 여기서 $\bf{F}^{(e)} = \sum_a \bf{F}^{(e)}_a$ 는 외력(external force) 의 총합이다. 

$$
\dfrac{d^2}{dt^2} \sum_a m_a \bf{r}_a = \sum_i \bf{F}^{(e)}_a + \sum_{\substack{ a ,b \\ a \ne b }} \bf{F}_{ba} = \bf{F}^{(e)}  + 0
...
$$
\bf{R} = \dfrac{\sum_a m_a \bf{r}_a } {\sum_a m_a} = \dfrac{\sum_a m_a \bf{r}_a}{M}\;.
$$ {#eq-CM_center_of_mass}

여기서 $M=\sum_a m_a$ 는 시스템의 입자의 질량의 총 합이며 @eq-CM_system_of_particles 으로 부터 다음을 얻는다.
...
$$
\bf{P}:=\sum_a m_a \dfrac{d\bf{r}_a}{dt} = M \dfrac{d\bf{R}}{dt}
...
$$
\begin{aligned}
\dfrac{d\bf{L}}{dt} &= \sum_a \dfrac{d}{dt}( \bf{r}_a \times \bf{p}_a) \\
&= \sum_a \dot{\bf{r}}_a \times \bf{p}_a + \bf{r}_a \times \dot{\bf{p}}_a = \sum_i \bf{r}_a \times \bf{F}^{(e)}_a +  \sum_{\substack{ a ,b \\ a \ne b }} \bf{r}_a \times \bf{F}_{ba} 
\end{aligned}
$$ {#eq-CM_derivative_of_total_angular_momentum}

이다. 여기서 $\dot{\bf{r}}_a \times \bf{p}_a = 0$ 이 이용되었다. 뉴턴의 제 3 운동법칙을 따르는 힘 이라면, 즉 $\bf{F}_{ab}= -\bf{F}_{ba}$ 라면, $\bf{r}_{ab} = \bf{r}_a - \bf{r}_b$ 에 대해

$$
\bf{r}_a \times \bf{F}_{ba} + \bf{r}_b \times \bf{F}_{ab} = (\bf{r}_a - \bf{r}_b) \times \bf{F}_{ba} = \bf{r}_{ab} \times \bf{F}_{ba}
$$

로 쓸 수 있다.  

여기서 작용 반작용의 강한 조건, 즉 $\bf{F}_{ba}=F_{ba}\hat{\bf{r}}_{ab}$ 일 경우를 생각하자. 이 경우 @eq-CM_derivative_of_total_angular_momentum 의 마지막 항은 아래와 같이 $0$ 이 된다.

$$
\sum_{\substack{ a ,b \\ a \ne b }} \bf{r}_a \times \bf{F}_{ba} = \sum_{a<b} \bf{r}_{ab}\times \bf{F}_{ba} = 0\;.
$$

그리고 외력 $\bf{F}^{(e)}$에 의한 토크를 $\displaystyle \bf{N}^{(e)}=\sum_a \bf{r}_a \times \bf{F}_a^{(e)}$ 로 정의하면 다음이 성립한다.
...
총 운동량 보존은 $\bf{F}_{ab} = -\bf{F}_{ba}$ 만을 가정하였다. 이를 작용-반작용의 약한 조건(Weak form of the action and reaction principle) 라고 한다. 그러나 총 각운동량 보존은 $\bf{F}_{ab} = F_{ab}\hat{\bf{r}}_{ab}$, 즉 중심력(central force) 임을 가정하였다 이를 작용-반작용의 강한 조건 이라고 한다.  
...
$N$-입자계에서 질량중심을 $\bf{R}=\sum\limits_{i=a}^N m_a \bf{r}_a$ 로 정의했다. $\bf{r}'_a=\bf{r}_a - \bf{R}$ 이라고 하면 $\bf{r}'_a$ 는 질량 중심 $\bf{R}$ 을 원점으로 삼은 위치벡터이다. 이 때 질량 중심의 속도 $\bf{V} = \dot{\bf{R}}$ 에 대해 $\bf{v}'_a = \bf{v}_a -\bf{V}$ 로 할 수 있다. 그렇다면 총 각운동량은 다음과 같다.

$$
\begin{aligned}
\bf{L} &= \sum_a \bf{r}_i \times m_a \bf{v}_a  \\
&=\sum_a \bf{R} \times m_a \bf{V} + \sum_a \bf{r}'_a  \times m_i \bf{v}'_a  \\[0.3em]
& \qquad + \left(\sum_a m_a \bf{r}'_a \right) \times \bf{V} + \bf{R} \times \left( \dfrac{d}{dt} \sum_a m_a \bf{r}'_a\right) \\[0.3em]
& = \bf{R} \times M\bf{V} + \sum_a \bf{r}'_a \times \bf{p}'_a\;.
\end{aligned}
$$ {#eq-CM_total_angular_momentum_for_center_of_mass_coord}

즉 원점 $\mathcal{O}$ 에서의 입자계의 총 각운동량 $\bf{L}$ 은 ($1$) 모든 질량이 질량 중심 에 집중되어 있다고 가정할 때의 각운동량 $\bf{L}_C= \bf{R}\times (M\bf{V})$ 와 ($2$) 질량중심을 새로운 원점으로 했을때의 각운동량 $\bf{L}_R=\sum_a \bf{r}_a' \times \bf{p}'_i$ 의 합이다. 
...
$$ 
W_{12} = \sum_i \int_{P_1}^{P_2} \bf{F}_a \cdot d\bf{s}_a = \sum_i  \int_{P_1}^{P_2} \bf{F}_a^{(e)}\cdot d\bf{s}_a + \sum_{\substack{ a,b \\ a \ne b }}  \int_{P_1}^{P_2}\bf{F}_{ba}\cdot d\bf{s}_a\;.
$$ {#eq-work_system_of_particles}

단일 입자의 경우를 생각하면,

$$
\sum_a \int_{P_1}^{P_2} \bf{F}_a \cdot d\bf{s}_a = \sum_i \int_{P_1}^{P_2} m_a \bf{v}_a \cdot \bf{v}_a dt = \sum_a \int_{P_1}^{P_2} d\left(\dfrac{1}{2} m_a v_a^2\right) = T_2 - T_1
$$
...
$$
T=\frac{1}{2} \sum_a m_a (\bf{V} + \bf{v}'_a) \cdot (\bf{V} + \bf{v}'_a)  = \dfrac{1}{2} MV^2 + \dfrac{1}{2} \sum_i m_a {{v'}_a}^2.
...
$$
\sum_a \int_{P_1}^{P_2} \bf{F}_a^{(e)} \cdot d\bf{s}_a = - \sum_a \int_{P_1}^{P_2} (\nabla_a V_a) \cdot d\bf{s}_a = - \left. \sum_a V_i\right|_{P_1}^{P_2} = \sum_a V_a (P_1) - V_i(P_2)
$$

이다. 입자 상호간의 힘 역시 보존력이라면, $\bf{F}_{ba}$ 와 $\bf{F}_{ab}$ 가 포텐셜 $V_{ab}$ 로 부터 얻어 질 수 있다. 강한 작용-반작용 조건에서 $V_{ab} = V(|\bf{r}_a -\bf{r}_b|)$ 이며 

$$
\bf{F}_{ba} = -\nabla_i V_{ab} = + \nabla_b V_{ab} = -\bf{F}_{ab}
$$

이므로 $\bf{F}_{ba}$ 와 $\bf{F}_{ab}$ 모두 두 입자의 위치를 잇는 선상에 위치한다. 즉 어떤 스칼라 함수 $f$ 에 대해, $V_{ab}$ 는 다음과 같은 형태이다.

$$
\nabla V_{ab}(|\bf{r}_a - \bf{r}_b|) = (\bf{r}_b - \bf{r}_a) f.
$$

만약 $V_{ab}$ 가 $\bf{r}_a - \bf{r}_b$ 이외의 다른 물리량에 의존한다면, 예를들어 $a$ 번째 입자의 속도 $\bf{v}_a$ 에 대해 $V_{ab} = V(|\bf{v}_a - \bf{v}_b |)$ 이거나 $a$ 번째 입자의 spin $\bf{S}_a$ 에 대해 $V(|\bf{S}_a - \bf{S}_b|)$ 꼴이라면 $\bf{F}_{ab} = -\bf{F}_{ba}$ 의 작용 반작용의 약한 조건은 만족하지만 강한 조건은 만족하지 않는다. 

입자간의 힘이 보존력이라면 @eq-work_system_of_particles 의 두번째 항은 $\nabla_{ab} V_{ab}=\nabla_a V_{ab} = -\nabla_b V_{ab}$, $d\bf{r}_{ab} = d\bf{s}_a - d\bf{s}_b = d\bf{r}_a -d\bf{r}_b$ 를 이용하면

$$
\begin{aligned}
\sum_{\substack{ a ,b \\ a \ne b }} \int_{P_1}^{P_2} \bf{F}_{ba}\cdot d\bf{s}_a  &= - \dfrac{1}{2} \sum_{\substack{ a ,b \\ a \ne b }} \int_{P_1}^{P_2} \left( \nabla_a V_{ab} \cdot d\bf{s}_ia+ \nabla_b V_{ab} \cdot d\bf{s}_b\right)  \\
&= -\dfrac{1}{2}\sum_{\substack{ a ,b \\ a \ne b }} \int_{P_1}^{P_2} \nabla_{ab} V_{ab} \cdot d\bf{r}_{ab} = -\dfrac{1}{2} \left. \sum_{\substack{ a ,b \\ a \ne b }}  V_{ab}\right|_{P_1}^{P_2}
\end{aligned}
$$
...
$$
V=\sum_a V_b + \dfrac{1}{2} \sum_{\substack{ a ,b \\ a \ne b }} V_{ab}\;.
$$
::: 
...
**구속조건(Constraint)** 는 운동방정식의 제한조건을 의미한다. 예를 들어 강체의 경우 $|\bf{r}_a - \bf{r}_b| =c_{ab} = \text{const.}$
...
시스템의 **가상 번위 (virtual displacement)** 는 주어진 시간 $t$ 에서 주어진 힘과 구속조건에서 반경벡터의 가능한 미소 변화(infinitesimal change of the coordinate) $\delta \bf{r}_a$ 에 의한 짜임새 공간(configuration space) 상의 미소 변화를 의미한다. 가상(virtual)이란 이름이 붙은 이유는 물리적으로 실제의 변위는 특정 시간 간격에서 발생하지만 가상변위는 시간 $t$ 가 고정되어 있기 때문이다.

:::

:::

시스템이 평형상태일 경우를 가정하자. 각 입자에 작용하는 힘 $\bf{F}_i = 0$ 이며 따라서 $\bf{F}_a \cdot \delta \bf{r}_i=0$ 이다. 또한 $\sum\limits_a \bf{F}_a \cdot \delta \bf{r}_a = 0$ 임은 자명하다. $\bf{F}_a$ 를 applied force $\bf{F}^{(a)}_a$ 와 구속력 $\bf{f}_a$ 로 분리 할 수 있다. 즉 $\bf{F}_a = \bf{F}^{(a)}_a + \bf{f}_a$ 이며 따라서 다음이 성립한다.

$$
\sum_a \bf{F}_a \cdot \delta \bf{r}_a = \sum_a \bf{F}^{(a)}_a \cdot \delta \bf{r}_a + \sum_a \bf{f}_a \cdot \delta \bf{r}_a = 0\;.
$$


이제 구속력에 의한  **가상 일(virtual work)** 이 $0$ 인 시스템, 즉 $\sum_a \bf{f}_a \cdot \delta \bf{r}_a=0$ 인 시스템으로 제한해보자. 이 조건은 강체를 비롯한 많은 다른 제약 조건에 대해서도 유효하다는 것을 안다. 예를 들어 입자가 표면 위를 움직이도록 제약되어 있다면, 구속력은 표면에 수직이며, 가상 변위는 표면에 접선이어야 하므로 가상 일은 $0$ 이다. 하지만 미끌어지는(sliding) 마찰력이 존재할 경우 가상 일이 $0$ 이 아니게 되는데 이런 시스템은 지금은 생각하지 않기로 하자. 이 제한은 마찰이 본질적으로 거시적인 현상이라는 것을 고려하면 우리의 물리를 해치지 않는다. 반면에 구르는(rolling) 구속조건은 이 조건을 위반하지 않는다. 왜냐하면 잠시 멈춘 지점에 작용하는 힘은 구속조건에 부합하는 변위에 대해 가상 일 이 $0$ 이기 때문이다. 또한 입자가 시간에 따라 움직이는 표면에 제약되어 있는 경우, 구속력은 각 순간마다 표면에 수직이며, 가상 일 은 $0$ 이지만 실제 시간 $dt$ 동안의 실제 변위에 의한 일은 $0$ 이 아닐 수 있다.

따라서 우리는 applied forces에 의한 가상 일이 $0$ 이 되는 계의 평행 조건을 얻게 된다. 즉
$$
\sum_a \bf{F}^{(a)}_a \cdot \delta \bf{r}_a = 0
$$ {#eq-CM_principle_of_virtual_work}

의 조건을 얻게 되며, 위의 @eq-CM_principle_of_virtual_work 은 **가상 일의 원리(the principle of virtual work)** 이라 불린다. $\delta \bf{r}_i$ 들은 서로 구속조건에 묶여 있기 때문에 서로 독립이 아니며 따라서 그 계수인 $\bf{F}^{(a)}_a$ 도 모두 $0$ 이 아니어도 된다(선형대수학을 생각하라). 즉 일반적으로 $\bf{F}^{(a)}_a \ne \bf{0}$ 이다.  


이렇게 되면 각각의 $i$ 번째 입자에 대해 $\bf{F}^{(a)}_a$ 가 $0$ 인 조건이 아니게 되며, 따라서 $\delta \bf{r}_a$ 는 서로 독립적이지 않게 되고 위의 @eq-CM_principle_of_virtual_work 가 하나의 구속조건이며 따라서 일반화된 좌표계에 대한 가상 변위로 변환해야 한다. @eq-CM_principle_of_virtual_work 는 구속력 $\bf{f}_a$ 를 포함하지 않는다는 측면에서 우리가 원하는 것이지만 단지 정적인 경우만을 다룬다는 점에서 부족하다. 우리는 이것으로 부터 우리가 원하는 운동방정식을 얻고자 한다.

</br>

### 달랑베르 원리(D'Alembert's principle) {#sec-CM_dlambert_principle}

운동방정식 $\bf{F}_a = \dot{\bf{p}}_a$ 로부터 시작하자. $\bf{F}_a - \dot{\bf{p}}_a=\bf{0}$ 이며 이것은 시스템의 입자들이 실제 힘 $\bf{F}$ 와 소위 *reversed effecvive force* 인 $-\dot{\bf{p}}_a$ 의 합에 의해 평행상태에 있게 된다는 의미이다. 여기에 가상변위를 적용하면

$$
\sum_a \left( \bf{F}_a - \dot{\bf{p}}_a \right) \cdot \delta \bf{r}_a = 0
$$

라고 쓸 수 있다. $\bf{F}_a = \bf{F}^{(a)}_a + \bf{f}_a$ 로 놓으면

$$
\sum_a \left(\bf{F}^{(a)} - \dot{\bf{p}}_a\right) \bf{\cdot}\delta \bf{r}_a + \sum_i \bf{f}_a \bf{\cdot}\delta \bf{r}_a = 0
$$

이며 구속력에 의한 가상 일이 $0$ 이라는 조건을 입히면,

$$
\sum_a \left( \bf{F}^{(a)}_a - \dot{\bf{p}}_a \right) \cdot \delta \bf{r}_a = 0\;
...
일반화된 좌표계 $(q_1,\,q_2,\ldots,\,q_n)$ 을 생각하자. $\bf{r}_a = \bf{r}_a (q_1,\ldots,\,q_n,\,t)$ 이므로 미분의 연쇄 규칙에 의해

$$
\bf{v}_a = \dfrac{d\bf{r}_a}{dt} = \sum_k \dfrac{\partial \bf{r}_a}{\partial q_k} \dot{q}_k + \dfrac{\partial \bf{r}_a}{\partial t} 
$$

이다. 우리는 이로부터

$$
\boxed{\dfrac{\partial \bf{v}_a}{\partial \dot{q}_k} = \dfrac{\partial \bf{r}_a}{\partial q_k}}
$$ {#eq-CM_dlambert_1}

임을 알 수 있다. 가상 변위 $\delta \bf{r}_a$ 는 정의에 의해 시간 $t$ 에 대한 편미분 항이 없으며 다음과 같다.

$$
\boxed{\delta \bf{r}_a = \sum_j\dfrac{\partial \bf{r}_a}{\partial q_j} \delta q_j.}
$$ {#eq-CM_virtual_displacement_and_generalized_coordinate}

이제, 

$$
\sum_a \bf{F}_a \cdot \delta \bf{r}_a = \sum_{a,\,j} \bf{F}_a \cdot \left( \dfrac{\partial \bf{r}_a}{\partial q_j}\right) \delta q_j = \sum_j Q_j \delta q_j
...
$$
\boxed{
Q_j = \sum\limits_a \bf{F}_a \cdot \dfrac{\partial \bf{r}_a}{\partial q_j}
}
...
달랑베르 원리(@eq-CM_dalembert_principle) 는 $\sum\limits_a \dot{\bf{p}}_a \bf{\cdot} \delta \bf{r}_a = \sum\limits_a m_a \ddot{\bf{r}}_a \bf{\cdot} \delta \bf{r}_a$ 로 쓸 수 있으며 

$$
\sum\limits_a m_a \ddot{\bf{r}}_a \bf{\cdot} \delta \bf{r}_a = \sum\limits_{a,\,j} m_a \ddot{\bf{r}}_a \cdot \dfrac{\partial \bf{r}_a}{\partial q_j} \delta q_j
$$ {#eq-CM_sec_lagrange_1}

이다. 또한

$$
\dfrac{d}{dt} \left(\dfrac{\partial \bf{r}_a}{\partial q_j} \right) = \sum_k \dfrac{ \partial^2 \bf{r}_a }{\partial q_j \partial q_k} \dot{q}_k + \dfrac{\partial^2 \bf{r}_a}{\partial q_j \partial t}  = \dfrac{\partial \dot{\bf{r}}_a}{\partial q_j} = \dfrac{\partial \bf{v}_a}{\partial q_j}
$$ {#eq-CM_sec_lagrange_2}

이다. 이로부터 @eq-CM_dlambert_1 과 @eq-CM_sec_lagrange_2 를 이용하면, 그리고 $T_a = m_a v_a^2/2$ 라고 놓으면,

$$
\begin{aligned}
\sum_{a} m_a \ddot{\bf{r}}_a \cdot \dfrac{\partial \bf{r}_a}{\partial q_j} &= \sum_a \left[\dfrac{d}{dt} \left(m_a \dot{\bf{r}}_a \bf{\cdot} \dfrac{\partial \bf{r}_a}{\partial q_j} \right)- m_a \dot{\bf{r}}_a \bf{\cdot} \dfrac{d}{dt}\left(\dfrac{\partial \bf{r}_a}{\partial q_j}\right)\right] \\[0.2em]
&= \sum_a \left[\dfrac{d}{dt} \left(m_a \bf{v}_a \bf{\cdot} \dfrac{\partial \bf{v}_a}{\partial \dot{q}_j} \right)- m_a \bf{v}_a \bf{\cdot} \dfrac{\partial \bf{v}_a}{\partial q_j}\right] \\[0.3em]
&= \sum_{a} \left[\dfrac{d}{dt} \dfrac{\partial}{\partial \dot{q}_j}\left(\dfrac{1}{2} m_a\|\bf{v}_a\|^2\right) - \dfrac{\partial}{\partial q_j}\left(\dfrac{1}{2}m_a \|\bf{v}_a\|^2\right)\right] \\[0.3em]
&= \sum_a \left[\dfrac{d}{dt}\left(\dfrac{\partial T_a}{\partial \dot{q}_j}\right) -\dfrac{\partial T_a}{\partial q_j}\right] 
\end{aligned}
$$

이다. 총 운동에너지 $T = \sum_a T_a$ 라고 놓으고 위 식과 @eq-CM_sec_lagrange_1 를 결합하면 다음을 얻는다.
...
$$
Q_j = \sum_a \bf{F}_a \cdot \dfrac{\partial \bf{r}_a}{\partial q_j} = -\sum_a (\nabla_i V) \cdot \dfrac{\partial \bf{r}_a}{\partial q_j} = - \dfrac{\partial V}{\partial q_j}\;.
$$
