...
우주에 입자(particle) 하나만 존재하며 당신은 이 입자에 영항을 끼치지 않으면서 위치와 속도를 잴 수 있는 존재라고 하자. 이 입자의 운동에 어떤 영향도 끼치는 존재가 없을 때 그 입자는 정지상태나 직선으로 균일한 운동상태, 즉 등속운동상태를 유지하는 좌표계가 존재한다는 말이며, 이것을 **관성좌표계(inertial frame)** 이라고 한다. 어떤 관성좌표계 $\mathcal{O}$ 에 대해 다른 좌표계의 원점 $\mathcal{O}'$ 이 $\bf{V} = (V_x,\,V_y,\,V_z)$ 의 일정한 속도로 움직인다고 하자. 처음의 좌표계의 radius vector $\bf{r} = (x,\,y,\,z)$ 라고 하고, 뒤의 좌표계의 위치 벡터 $\bf{r}'=(x',\,y',\, z')$ 라고 하면, 
...
제 3 운동법칙에 따라 모든 $i,\,j$ 에 대해 $\bf{F}_{ij} = - \bf{F}_{ji}$ 일 경우 모든 particle에 대해 더하면 다음과 같다. 여기서 $\bf{F}^{(e)} = \sum_i \bf{F}^{(e)}_i$ 는 external force 의 총합이다. 
...
총 운동량 보존은 $\bf{F}_{ij} = -\bf{F}_{ji}$만을 가정하였다. 이를 작용-반작용의 약한 조건(Weak form of the action and reaction principle) 라고 한다. 그러나 총 각운동량 보존은. $\bf{F}_{ij} = F_{ij}\hat{\bf{r}}_{ij}$, 즉 central force 임을 가정하였다 이를 작용-반작용의 강한 조건 이라고 한다.  
...
입자계에서 특정 시간에서의 각 입자의 위치의 집합을 **구성공간(configuration space)** 이라고 한다. 이 구성공간은 $N$ 입자의 경우 $3N$ 차원 벡터공간이 된다. 입자계가 configuration 1($\mathcal{C}_1$) 에서 configuration 2 ($\mathcal{C}_2$) 로 변화했을때의 총 일 $W_{12}$ 는 다음과 같다.
$$ 
W_{12} = \sum_i \int_1^2 \bf{F}_i \cdot d\bf{s}_i = \sum_i \int_1^2 \bf{F}_i^{(e)}\cdot d\bf{s}_i + \sum_{\substack{ i ,j \\ i \ne j }} \int_1^2 \bf{F}_{ji}\cdot d\bf{s}_i\;.
$$ {#eq-work_system_of_particles}

단일 입자의 경우를 생각하면,

$$
\sum_i \int_1^2 \bf{F}_i \cdot d\bf{s}_i = \sum_i \int_1^2 m_i \bf{v}_i \cdot \bf{v}_i dt = \sum_i \int_1^2 d(\dfrac{1}{2} m_i v_i^2) = T_2 - T_1
$$
...
$$
\sum_i \int_1^2 \bf{F}_i^{(e)} \cdot d\bf{s}_i = - \sum_i \int_1^2 (\nabla_i V_i) \cdot d\bf{s}_i = - \left. \sum_i V_i\right|_1^2 = \sum_i V_i (1) - V_i(2)
$$
...
$$
\begin{aligned}
\sum_{\substack{ i ,j \\ i \ne j }} \int_1^2 \bf{F}_{ji}\cdot d\bf{s}_i  &= - \dfrac{1}{2} \sum_{\substack{ i ,j \\ i \ne j }} \int_1^2 \left( \nabla_i V_{ij} \cdot d\bf{s}_i + \nabla_j V_{ij} \cdot d\bf{s}_j\right)  \\
...
Constraint가 시간과 위치에 대한 함수이며 다음과 같은 꼴로 표현 될 수 있을 때 **holonomic** 이라 한다. 

$$
f(\bf{r}_1,\,\bf{r}_2,\ldots,\,t) =0.
$$ {#eq-CM_holonomic_constraint}

Holonomic constraint 가 아닌 constraint 는 **nonholonomic** 이라고 한다.

Constraint가 explicit한 시간 의존성이 없을 때, 즉 $\dfrac{\partial}{\partial t} f(\bf{r}_i;\, \dot{\bf{r}}_i ;\,t)=0$  일 때 **scleronomous constraint** 라 하며 아닐 때 **rheonomous constraint** 라 한다. 

</br>

### Constraints 와 일반화된 좌표계 {#sec-CM_generalized_coordinates}

물체의 운동을 기술할 때 constraints가 부여된다면 두가지 어려운 점이 발생한다. 우선 좌표 $(r_1,\,r_2,\ldots)$ 는 더이상 서로 독립적이지 하지 않다. 두번째로는 **구속력(force of constraint)** 문제가 있다. 

Holonomic 의 경우 소위 **일반화된 좌표계(generalized coordinate)** 를 사용함으로서 문제를 풀 수 있다. 

$N$-입자계를 생각하자. 구속조건이 없다면 $3N$ 개의 독립적인 좌표변수가 존재한다. 이렇게 시스템을 기술하는데 필요한 독립적인 변수의 갯수를 **자유도(degree of freedom)** 라고 한다. 만약 $k$ 개의 독립적인 holonomic constraints가 존재한다면 이 입자계의 자유도는 $3N-k$ 가 될 것이다. 그렇다면 이 입자계는 $3N-k$ 개의 독립변수 $q_1,\ldots,\,q_{3N-k}$ 로 기술 될 수 있을 것이다. 즉 $\bf{r}_i = \bf{r}_i (q_1,\ldots,\,q_{3N-k},t)$ for $i=1,\ldots,\,3N-k$. 이 경우 $q_i$ 가 거리 단위의 값일 필요가 없으며 어떤 $(q_i,\,q_j,\,q_k)$ 가 벡터일 필요도 없다. *모든 종류의 물리량이 일반화된 좌표계의 좌표변수가 될 수 있다.* Nonhonolomic 구속조건의 경우는 독립변수의 개수를 줄이는 데 사용 될 수 없다.
...
시스템의 **가상 번위 (virtual displacement)** 는 주어진 시간 $t$ 에서 주어진 힘과 구속조건에서 가능한 좌표의 미소 변화(infinitesimal change of the coordinate) $\delta \bf{r}_i$ 에 의한 구성공간(configuration space) 상의 미소 변화를 의미한다. virtual이란 이름이 붙은 이유는 물리적으로 실제의 변위는 특정 시간 간격에서 발생하지만 가상변위는 시간 $t$ 가 고정되어 있기 때문이다.
...
이렇게 되면 각각의 $i$ 번째 입자에 대해 $\bf{F}^{(a)}_i$ 가 $0$ 인 조건이 아니게 되며, 따라서 $\delta \bf{r}_i$ 는 서로 독립적이지 않게 되고 위의 @eq-CM_principle_of_virtual_work 가 하나의 constraint 이며 따라서 일반화된 좌표계에 대한 가상 변외로 변환해야 한다. @eq-CM_principle_of_virtual_work 는 constraint force $\bf{f}_i$ 를 포함하지 않는다는 측면에서 우리가 원하는 것이지만 단지 statics 만을 다룬다는 점에서 부족하다. 우리는 이것으로 부터 우리가 원하는 운동방정식을 얻고자 한다.

</br>

### 달랑베르 원리(D'Alembert's principle) {#sec-CM_dlambert_principle}

운동방정식 $\bf{F}_i = \dot{\bf{p}}_i$ 로부터 시작하자. $\bf{F}_i - \dot{\bf{p}}_i=\bf{0}$ 이며 이것은 시스템의 입자들이 실제 힘 $\bf{F}$ 와 *reversed effecvive force* $-\dot{\bf{p}}_i$ 의 합에 의해 평행상태에 있게 된다는 의미이다. 여기에 가상 변외를 적용하면
...
### Lagrange's equations {#sec-CM_lagrange_equation}
