...
이 방정식의 해를 구하기 위해서는 $2n$ 개의 초기값이 필요하며 ($1$) 특정 시간 $t$ 에 대해 $n$ 개의 $q_i(t)$ 와 역시 $n$ 개의 $\dot{q}(t_i)$ 이거나 ($2$) 두 시간 $t_1,\,t_2$ 에 대해 $n$ 개의 $q_i(t_1)$ 과 $q_i(t_2)$ 이다. 즉 $n$ 개의 독립 변수 $\{q_i\}$ 와 시간에 대한 미분 $\{\dot{q}_i\}$ 에 관한 문제이다. 라그랑지안은  각각의 일반화된 좌표로 이루어진 $n$ 차원 구성 공간(configuration space) 에서 표현된다.

이에 반해 $n$-자유도 시스템의 해밀토니안 역학은 $2n$ 개의 독립변수에 대한 $2n$ 개의 1차 이분방정식으로 기술된다. 필요한 초기조건은 라그랑지안과 동일한 $2n$ 개이며, $2n$ 차원의 위상 공간(phase space) 에서 표현된다. 이 $2n$ 개의 독립변수는 $n$ 개의 일반화된 변수 $\{q_i\}$ 와 $n$ 개의 정준 운동량 $\{p_i\}$ 이며 $(\boldsymbol{q},\, \boldsymbol{p})$ 를 **정준 변수 (canonical variables)** 라고 한다. 
...
1. 적절한 (일반화된) 좌표계에서 라그랑지안 $L(\boldsymbol{q},\, \dot{\boldsymbol{q}},\,t)$ 를 구한다.

2. $q_i$ 에 대한 정준 운동량 $p_i = \dfrac{\partial L}{\partial \dot{q}_i}$ 를 구한다.
...
@sec-CM_cyclic_coordinate_lagrangian 에서 다루었던 순환 좌표(cyclic coordinate) 를 다시 생각해보자. 라그랑지안과 해밀토니안의 관계 즉 $H = \sum_i \dot{q}_i p_i -L$ 을 생각해보면
...
이다. 여기서 라그랑지안에 대한 $S_L$ 는 구성공간에서의 경로에 대한 적분이지만 해밀토니안에 대한 $S_H$ 는 위상공간에서의 경로에 대한 적분이 이 되며, 해밀토니안 형식에 따라 $\bf{q}$ 와 $\bf{p}$ 는 각각 독립변수로 간주된다. 따라서 @eq-CM_action_in_phase_space 에서 적분되는 함수는 $\bf{q},\, \dot{q},\, \bf{p},\, \dot{\bf{p}}$ 의 함수로 표현되어야 한다. 따라서 어떤 경로의 variation 역시 위상공간에서의 경로의 variation 이다. 해밀턴의 원리는 구성공간에서의 action $S_L$ 에 대해 다루는데 **변형된 해밀턴의 원리 (modified Hamilton's principle)** 는 위상공간에서의 action $S_H$ 에 대해 $\delta S_H=0$ 이 되는 위상공간 상의 경로를 따른는 것을 주장한다. 즉
