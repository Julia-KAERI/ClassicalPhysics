...
$$ {#eq-time_dependence_of_hamiltonian} 

를 얻는다. @eq-canonical_equation_of_hamiltonian 을 **해밀턴의 정준 방정식(canonical equations of Haminton)** 이라고 부른다. 우리는 이를 통해 $n$ 개의 2차 미분방정식(라그랑쥬 방정식) 을 $2n$ 개의 1차 미분방정식(해밀턴의 정준 방정식) 으로 바꿨다. 차이는 (이것은 매우 중요하다) 라그랑지안이 일반화된 좌표계에서 $\{q_i\},\, \{\dot{q}_i\},\,t$ 의 함수라면 해밀토니안은 $\{q_i\},\, \{p_i\},\,t$ 의 함수라는 것이다.
...
$$ {#eq-homogeneous_function}

이 성립한다는 의미다. 이 경우,

$$
k f(x_1,\ldots,\,x_n) = \sum_{i=1}^n x_i \dfrac{\partial f}{\partial x_i}
$$ {#eq-differential_of_homogeneous_function}
...
이다. 이제 이것을 전자기장에서의 라그랑지안에 대해 생각해보자. @eq-generalized_force 으로 주어진 일반화된 포텐셜에 대해 
...
$$ {#eq-hamiltonian_of_charged_particle}
...
그렇다면 해밀턴의 정준방정식 @eq-canonical_equation_of_hamiltonian 은 다음과 같이 쓸 수 있다.
...
@sec-cyclic_coordinate_lagrangian 에서 다루었던 cyclic coordinate 를 다시 생각해보자. 라그랑지안과 해밀토니안의 관계 즉 $H = \sum_i \dot{q}_i p_i -L$ 을 생각해보면
...
또한 해밀턴 운동방정식 @eq-canonical_equation_of_hamiltonian 과 @eq-time_dependence_of_hamiltonian 으로부터
$$
...
[해밀턴의 원리](02_variational_principle.qmd#sec-least_action_theorem) 를 상기하자. 라그랑쥬 방정식은 해밀턴의 원리로부터의 자연스러운 결론이다. 이제 해밀턴의 원리로부터 해밀턴의 운동방정식이 자연스럽게 유도됨을 보이고자 한다. $L = \sum_i \dot{q}_i p_i - H(\{q_i\},\, \{p_i\},\,t)$ 이므로 action $S$ 는
...
일 것이다. 우리가 원하는 것은 이 변환에 대해 @eq-canonical_equation_of_hamiltonian 와 같은 해밀턴 운동방정식이 만족되는 것이다. 즉 변환된 해밀토니안을 $\overline{H}$ 라고 하면
